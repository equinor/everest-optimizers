...
#
def readBuildStatsCsvFileIntoDictOfLists(
    buildStatusCsvFileName,
    computeStdScaledFields=True,
    normalizeFileName=True,
):
    buildStatsDOL = readCsvFileIntoDictOfLists(
        buildStatusCsvFileName, getStdBuildStatsColsAndTypesList(),
    )
...
def readCsvFileIntoDictOfLists(csvFileName, colNameAndTypeList):
    ...
    with open(csvFileName) as csvFile:
        ...
        columnHeadersList = CDQAR.getColumnHeadersFromCsvFileReader(
            csvFileName, csvReader,
        )
        if len(columnHeadersList) == 0:
            # File is empty so just return an empty distOfLists!
            return dictOfLists
        colNameTypeIdxList = getColNameTypeIdxListGivenColNameAndTypeList(
            csvFileName, columnHeadersList, colNameAndTypeList,
        )
        ...
        for lineList in csvReader:
            ...
            assertNumExpectedCsvFileLineEntries(
                csvFileName, columnHeadersList, dataRowIdx, lineList,
            )
...
def assertNumExpectedCsvFileLineEntries(
    csvFileName,
    columnHeadersList,
    dataRowIdx,
    csvLineList,
...
def addStdScaledBuildStatsFields(buildStatsDOL):
    addNewFieldByScalingExistingField(
        buildStatsDOL, "max_resident_size_Kb", 1.0 / 1024, 2, "max_resident_size_mb",
    )
    addNewFieldByScalingExistingField(
        buildStatsDOL, "FileSize", 1.0 / (1024 * 1024), 2, "file_size_mb",
    )


# Scale an existing field to create a new field
#
def addNewFieldByScalingExistingField(
    dictOfLists,
    existingFieldName,
    scaleFactor,
    decimalPlaces,
    ...
    while row_idx < numTotalRows:
        ...
        for baseDir in binBySubdirsUnderDirsList:
            if fileName.startswith(baseDir + "/"):
                ...
                fileNameBuildStatsDOL = binnedBuildStatsDOL_dict.setdefault(
                    subdirUnderBaseDir, {},
                )
                addRowFromDictOfListsToDictOfLists(
                    buildStatsDOL, row_idx, fileNameBuildStatsDOL,
                )
        row_idx += 1
    return BuildStatsBinnedBySubdirs(buildStatsDOL, binnedBuildStatsDOL_dict)


class BuildStatsBinnedBySubdirs:
...
def computeStdBuildStatsSummaries(buildStatsBinnedBySubdirs):
    fullBuildStatsSummariesList = computeStdBuildStatsSummariesSingleDOL(
    ...
    for subdir in buildStatsBinnedBySubdirs.binnedBuildStatsDOL_dict.keys():
        binnedBuildStatsSummariesList_dict.update(
            {
                subdir: computeStdBuildStatsSummariesSingleDOL(
                    buildStatsBinnedBySubdirs.binnedBuildStatsDOL_dict.get(subdir),
                ),
            },
        )
    return BuildStatsSummariesBinnedBySubdirs(
...
#
def createAsciiReportOfBuildStatsSummariesSingleSet(
    buildStatsSummariesList,
    buildStatsSetName,
):
    asciiReportStr = ""
    for buildStatsSummary in buildStatsSummariesList:
        asciiReportStr += createAsciiReportOfOneBuildStatsSummary(
            buildStatsSummary, buildStatsSetName,
        )
...
def createAsciiReportOfBuildStatsSummaries(buildStatsSummariesBinnedBySubdirs):
    ...
    asciiReportStr += createAsciiReportOfBuildStatsSummariesSingleSet(
        buildStatsSummariesBinnedBySubdirs.fullBuildStatsSummariesList, "Full Project",
    )
    binnedBuildStatsSummariesList_dict = (
...
def injectCmndLineOptionsInParser(clp):
    clp.add_argument(
        "--bin-by-subdirs-under-dirs",
        dest="binBySubdirsUnderDirsStr",
        default="",
