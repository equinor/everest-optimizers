...
def readBuildStatsTimingFileIntoDictTest(
    testObj,
...
class test_readBuildStatsTimingFileIntoDict(unittest.TestCase):
    def test_correct(self):
        ...
        errMsg_expected = ""
        readBuildStatsTimingFileIntoDictTest(
            self,
            buildStatsTimingFile,
            numKeys_expected,
            buildStatsTimingDict_expected,
    ...
    def test_missing_fail(self):
        buildStatsTimingFile = g_testBaseDir + "/file_does_not_exist.timing"
        numKeys_expected = 0
    ...
    def test_junk_fail(self):
        ...
        errMsg_expected = (
            buildStatsTimingFile
            + ": ERROR: Error, for CSV file"
            + " '"
            + buildStatsTimingFile
            + "' the data row 0 ['for this garbage'] has 1 entries"
    ...
    def test_missing_col_filename_fail(self):
        buildStatsTimingFile = (
            g_testBaseDir
            + "/bad_timing_build_stats_files/target1.timing.missing_col_filename"
        )
        numKeys_expected = 0
        buildStatsTimingDict_expected = None
        errMsg_expected = (
    ...
    def test_bad_type_filesize_fail(self):
        buildStatsTimingFile = (
            g_testBaseDir
            + "/bad_timing_build_stats_files/target1.timing.bad_type_filesize"
        )
        numKeys_expected = 0
        buildStatsTimingDict_expected = None
        errMsg_expected = (
            buildStatsTimingFile
            + ": ERROR: For field 'FileSize' the string value"
...
def gather_build_stats_py_expected_output(csvFile):
    return (
        "Reading all *.timing files from under '"
        + g_testBaseDir
        + "/dummy_build_dir' ...\n"
        + "Number of *.timing files found = 4\n"
        + g_testBaseDir
        + "/dummy_build_dir/some/base/target3.timing: ERROR: Contains 0 != 1 data rows!\n"
...
def test_gather_build_stats_py_body(testObj, csvFile, cmnd, silentStdout=False):
    ...
    with open(csvFile) as csvFileHandle:
        csvFileText = csvFileHandle.read()
    testObj.assertEqual(
        sortCsvFileTextList(csvFileText), sortCsvFileTextList(csvFileText_expected),
    )
    if not silentStdout:
        testObj.assertEqual(
            output.split("\n"),
...
class test_gather_build_stats_py(unittest.TestCase):
    def test_help(self):
        ...
        # print("output:\n"+output+"\n")
        self.assertTrue(
            output.find("Gather up build stats from *.timing CSV files") != -1,
        )
        self.assertTrue(output.find("max_resident_size_Kb : float") != -1)
        self.assertTrue(output.find("FileName : string") != -1)
        self.assertTrue(
            output.find("The column headers in all of the *.timing files are combined")
            != -1,
        )
