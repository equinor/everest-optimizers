...
class test_ColNameTypeIdx(unittest.TestCase):
    ...
    def test_invalid_type(self):
        threwExcept = True
        try:
            colNameTypeIdx = SBS.ColNameTypeIdx(
                SBS.ColNameAndType("name", "invalid"), 2,
            )
            threwExcept = False
        except Exception as errMsg:
            self.assertEqual(
...
class test_addNewFieldByScalingExistingField(unittest.TestCase):
    def test_add_field_1(self):
        ...
        SBS.addNewFieldByScalingExistingField(
            dictOfLists, "field_1", 0.1, 2, "scaled_field",
        )
...
class test_binBuildStatsDictOfListsBySubdirUnderDirs(unittest.TestCase):
    def test_1(self):
        ...
        self.assertEqual(
            buildStatsBinnedBySubdirs.fullBuildStatsDOL, dummy1BuildStatsDOL,
        )
    ...
    def test_2(self):
        ...
        self.assertEqual(
            buildStatsBinnedBySubdirs.fullBuildStatsDOL, dummy2BuildStatsDOL,
        )
        binnedBuildStatsDOL_dict = buildStatsBinnedBySubdirs.binnedBuildStatsDOL_dict
        self.assertEqual(len(binnedBuildStatsDOL_dict.keys()), 3)
...
class test_computeBuildStatusSummaryForOneField(unittest.TestCase):
    def test_field_1(self):
        ...
        buildStatSummary = SBS.computeBuildStatusSummaryForOneField(
            buildStatsDOL, "max_resident_size_mb", 2,
        )
...
class test_computeStdBuildStatsSummariesSingleDOL(unittest.TestCase):
    def test_big_small(self):
        ...
        self.assertEqual(
            bssl[2].maxValue, SBS.roundNum(45000000 / (1024.0 * 1024.0), 2),
        )
        self.assertEqual(
            bssl[2].maxFileName,
...
class test_computeStdBuildStatsSummaries(unittest.TestCase):
    def test_big_small(self):
        ...
        self.assertEqual(
            len(
                buildStatsSummariesBinnedBySubdirs.binnedBuildStatsSummariesList_dict.keys(),
            ),
            6,
        )
        self.assertEqual(
        ...
        bssl = buildStatsSummariesBinnedBySubdirs.binnedBuildStatsSummariesList_dict[
            "adelus"
        ]
        ...
        self.assertEqual(
            bssl[0].sumValue, SBS.roundNum((680000 + 35000 + 64000 + 77000) / 1024.0, 2),
        )
        self.assertEqual(bssl[0].maxValue, SBS.roundNum(680000 / 1024.0, 2))
        self.assertEqual(
            bssl[0].maxFileName,
            "packages/adelus/src/CMakeFiles/zadelus.dir/Adelus_pcomm.cpp.o",
        )
...
class test_createAsciiReportOfBuildStatsSummariesSingleSet(unittest.TestCase):
    def test_big_small(self):
        buildStatsDOL = getBuildStatsForTests()
        buildStatsSummariesList = SBS.computeStdBuildStatsSummariesSingleDOL(
            buildStatsDOL,
        )
...
big_small_summary_full_project_ascii = r"""Full Project: sum(max_resident_size_mb) = 10023.45 (21 entries)
Full Project: max(max_resident_size_mb) = 2343.75 (packages/tpetra/classic/NodeAPI/CMakeFiles/tpetraclassicnodeapi.dir/Kokkos_DefaultNode.cpp.o)
Full Project: sum(elapsed_real_time_sec) = 157.9 (21 entries)
Full Project: max(elapsed_real_time_sec) = 48.2 (packages/rol/adapters/epetra/test/sol/CMakeFiles/ROL_adapters_epetra_test_sol_EpetraSROMSampleGenerator.dir/test_02.cpp.o)
Full Project: sum(file_size_mb) = 157.19 (21 entries)
...
empty_summary_ascii = r"""No build statistics to summarize!"""
...
class test_createAsciiReportOfBuildStatsSummaries(unittest.TestCase):
    def test_big_small(self):
        ...
        self.assertEqual(buildStatsAsciiReport, big_small_summary_ascii)
...
class test_summarize_build_stats_py(unittest.TestCase):
    def test_big_small_full_project(self):
        cmnd = (
            ...
            + " "
            + g_testBaseDir
            + "/build_stats.big.small.csv"
        )
        output = GSS.getCmndOutput(cmnd)
        self.assertEqual(
            GSS.s(output), GSS.s(big_small_summary_full_project_ascii + "\n"),
        )
