...
def readAllValidTimingFiles(baseDir, printErrMsg=True, printStats=False):
    ...
    for timingFile in listOfAllTimingFiles:
        ...
        (buildStatsTimingDict, errMsg) = readBuildStatsTimingFileIntoDict(
            timingFileFullPath,
        )
    ...
    if printStats:
        print(
            "Number of valid *.timing files found = " + str(len(allValidTimingFilesLOD)),
        )
...
"""
    + getRequiredFieldsAndTypesDocStr()
    + r"""
...
if __name__ == "__main__":
    ...
    allValidTimingFilesListOfDicts = readAllValidTimingFiles(
        inOptions.baseDir, printStats=inOptions.verbose,
    )
    allValidTimingFilesDictOfLists = getDictOfListsFromListOfDicts(
        allValidTimingFilesListOfDicts, printStats=inOptions.verbose,
    )
    writeDictOfListsToCsvFile(
        allValidTimingFilesDictOfLists, inOptions.buildStatsCsvFile,
    )
    if inOptions.verbose:
