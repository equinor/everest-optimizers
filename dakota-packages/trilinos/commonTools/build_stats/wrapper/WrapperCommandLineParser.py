...
class WrapperCommandLineParser:
    ...
    def parse_env_controls(self):
        ...
        if "TRILINOS_BUILD_STATS_TIME_CMD" not in os.environ:
            err_msg += os.linesep
            err_msg += (
                "TRILINOS_BUILD_STATS_TIME_CMD (ENV) is required. CMake should "
        ...
        if "TRILINOS_BUILD_STATS_INNER_OP" not in os.environ:
            err_msg += os.linesep
            err_msg += (
                "TRILINOS_BUILD_STATS_INNER_OP (ENV) is required. CMake should "
        ...
        if "TRILINOS_BUILD_STATS_BASE_DIR" not in os.environ:
            err_msg += os.linesep
            err_msg += (
                "TRILINOS_BUILD_STATS_BASE_DIR (ENV) is required. CMake should "
        ...
        if parse_nm.lower() == "true":
            ...
            msg = f"ERROR: TRILINOS_BUILD_STATS_PARSE_NM is set to [{parse_nm}]"
            msg += f", but valid values are True or False. Defaulting to True{os.linesep}"
    ...
    def parse_cmdline_args(self, cmdline_args):
        ...
        try:
            ...
            if wrapper_arg == wrapper_header_arg:
                ...
                sys.stderr.write(
                    "----get_header was requested, but is not implemented"
                    ". Doing nothing.",
                )
                exit(0)
            ...
            print(
                "Got an error parsing the command line in the compiler wrapper python script",
            )
            print(e)
            raise
