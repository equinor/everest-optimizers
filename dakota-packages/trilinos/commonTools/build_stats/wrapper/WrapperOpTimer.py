...
usr_bin_time_csv_map = {
    "E": "elapsed_real_time_fmt",
    "e": "elapsed_real_time_sec",
    "S": "cpu_sec_kernel_mode",
    "U": "cpu_sec_user_mode",
    "P": "perc_cpu_used",
    "M": "max_resident_size_Kb",
    "t": "avg_resident_size_Kb",
    "K": "avg_total_memory_used_Kb",
    "D": "avg_size_unshared_data_area_Kb",
    "p": "avg_size_unshared_stack_area_Kb",
    "X": "avg_size_unshared_text_area_Kb",
    "Z": "page_size_bytes",
    "F": "num_major_page_faults",
    "R": "num_minor_page_faults",
    "W": "num_swapped",
    "c": "num_involuntary_context_switch",
    "w": "num_waits",
    "I": "num_filesystem_inputs",
    "O": "num_filesystem_outputs",
    "r": "num_socket_msg_recv",
    "s": "num_socket_msg_sent",
    "k": "num_signals",
    "x": "exit_status",
}

usr_bin_time_desc_map = {
    "E": "Elapsed real time ([h:]m:s)",
    "e": "Elapsed real time (s)",
    "S": "Total number of CPU-seconds that the process spent in kernel mode",
    "U": "Total number of CPU-seconds that the process spent in user mode",
    "P": "Percentage of the CPU that this job got",
    "M": "Maximum resident set size of the process during its lifetime (Kb)",
    "t": "(Not in tcsh.) Average resident set size of the process (Kb)",
    "K": "Average total (data+stack+text) memory use of the process (Kb)",
    "D": "Average size of unshared data area (Kb)",
    "p": "Average size of unshared stack space (Kb)",
    "X": "Average size of shared text space (Kb)",
...
field_header_full = get_full_header(
    default_fields, usr_bin_time_csv_map,
)  #','.join([ WrapperOpTimer.usr_bin_time_csv_map[f] for f in default_fields ])
field_header_short = ",".join(default_fields)
field_arg = (
    "--format="
    + field_header_full
    + "\n"
    + ",".join([f"%{f}" for f in default_fields])
)
...
class WrapperOpTimer:
    ...
    def time_op(wcp):
        ...
        # reading csv file
        with open(wcp.output_stats_file) as csvfile:
        ...
        if wcp.base_build_dir:
            ...
            rel_op_output_file = os.path.join(
                rel_path_to_base_build_dir, wcp.op_output_file,
            )
        else:
