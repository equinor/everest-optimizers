...
class NMParser:
    ...
    nm_re_str = (
        r"^[a-zA-Z0-9]+\s+(?P<size_hex>[a-zA-Z0-9]{2,})\s+(?P<type>["
        + nm_re_type_expr
        + r"])\s+"
    )
    ...
    def parse_object(filename):
        ...
        p = subprocess.Popen(
            ["nm", "-aS", filename], stdout=subprocess.PIPE, stderr=local_devnull,
        )
    ...
    @staticmethod
    def print_counts(nm_counts, cvs_line=False, csv_header=False):
        for k, v in nm_counts.items():
            print(f'"{NMParser.nm_option_desc_map[k]}",{v}')
    ...
    def get_csv_map(nm_counts):
        ...
        # parsed
        csv_map = {
            v: nm_counts.get(k, 0) for k, v in NMParser.nm_option_csv_map.items()
        }
        return csv_map
