...
def getColNameTypeIdxListGivenColNameAndTypeList(
    csvFileName,
    columnHeadersList,
    ...
    for colNameAndTypeToGet in colNameAndTypesToGetList:
        ...
        if colIdx != -1:
            ...
            raise Exception(
                "Error, the CSV file column header '"
                + colNameAndTypeToGet.colName()
                + "'"
                + " does not exist in the list of column headers "
...
class ColNameAndType:
    ...
    def convertFromStr(self, strIn):
        if self.__colType == "string":
            return strIn
        if self.__colType == "int":
            return int(strIn)
        if self.__colType == "float":
            return float(strIn)

    def assertType(self):
        supportedTypes = ["string", "int", "float"]
        if GSS.findInSequence(supportedTypes, self.__colType) == -1:
            raise Exception(
                "Error, type '"
                + str(self.__colType)
...
class ColNameTypeIdx:
    ...
    def __eq__(self, other):
        return (self.__colNameAndType, self.__colIdx) == (
            other.__colNameAndType,
            other.__colIdx,
        )
