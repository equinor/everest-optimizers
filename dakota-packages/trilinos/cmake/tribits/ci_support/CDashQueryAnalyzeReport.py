...
from Python2and3 import csvReaderNext, u


# Accept the --date input option with values 'today', 'yesterday', or some
# 'YYYY-MM-DD' value.
#
def convertInputDateArgToYYYYMMDD(
    cdashProjectTestingDayStartTime,
    dateText,
...
def validateAndConvertYYYYMMDD(dateText):
    try:
        ...
        raise ValueError(
            "Incorrect data format for '" + dateText + "', should be YYYY-MM-DD",
        )
    ...
    if hasSameKeyValuePairs:
        for key_1 in dict_1.keys():
            if key_1 not in dict_2.keys():
                hasSameKeyValuePairs = False
                errMsg = (
                    dict_1_name + "['" + key_1 + "'] does not exist in " + dict_2_name
                )
                break
...
def splitListOnMatch(inputList, matchFunctor):
    ...
    for ele in inputList:
        if matchFunctor(ele):
            matchList.append(ele)
        else:
            nomatchList.append(ele)
...
def cdashColorPassed():
    return "green"


def cdashColorFailed():
    return "red"


def cdashColorNotRun():
    return "orange"


def cdashColorMissing():
    return "gray"
...
def getStandardTestsetTypeInfo(testsetAcro, testsetColor=None):
    if testsetAcro == "twoif":
        ...
        tsti = TestsetTypeInfo(
            ...
            "pass",
            cdashColorPassed(),
            existanceTriggersGlobalFail=False,
        )
        ...
        raise Exception(
            "Error, testsetAcro = '" + str(testsetAcro) + "' is not supported!",
        )
...
def getTestsetAcroFromTestDict(testDict):
    ...
    raise Exception(
        ...
        + "' with fields"
        + " status = '"
        + str(testDict.get("status", None))
        + "' and"
        + " issue_tracker = '"
        + str(testDict.get("issue_tracker", None))
        + "'"
...
def isTestMissing(testDict):
    ...
    if status == "Missing":
        return True
    if status == "Missing / Failed":
        return True
    return False


# Define default test dicts sort order in tables
def getDefaultTestDictsSortKeyList():
    return ["testname", "buildName", "site"]
...
# expected headers.
#
def readCsvFileIntoListOfDicts(
    csvFileName,
    requiredColumnHeadersList=[],
    ...
    with open(csvFileName) as csvFile:
        ...
        assertExpectedColumnHeadersFromCsvFile(
            csvFileName,
            requiredColumnHeadersList,
            optionalColumnHeadersList,
            columnHeadersList,
        ...
        for lineList in csvReader:
            ...
            assertExpectedNumColsFromCsvFile(
                csvFileName, dataRow, lineList, columnHeadersList,
            )
...
def assertExpectedColumnHeadersFromCsvFile(
    csvFileName,
    requiredColumnHeadersList,
    optionalColumnHeadersList,
    columnHeadersList,
    ...
    for colHeader in columnHeadersList:
        if colHeader not in requiredAndOptionalHeadersSet:
            raise Exception(
                "Error, for CSV file '"
                + csvFileName
                + "' the"
                ...
                + str(colHeader)
                + "' is not in the set"
                + " of required column headers '"
                + str(requiredColumnHeadersList)
                + "'"
                + " or optional column headers '"
                + str(optionalColumnHeadersList)
                + "'!"
                + "",
            )
    ...
    for requiredHeader in requiredColumnHeadersList:
        if requiredHeader not in columnHeadersSet:
            raise Exception(
                "Error, for CSV file '"
                + csvFileName
                + "' the"
                ...
                + str(requiredHeader)
                + "' is missing from the"
                + " set of included column headers '"
                + str(columnHeadersList)
                + "'!"
                + "",
            )


def assertExpectedNumColsFromCsvFile(
    csvFileName,
    dataRow,
    lineList,
    columnHeadersList,
    ...
    if len(lineList) != len(columnHeadersList):
        raise Exception(
            ...
            + " "
            + str(len(lineList))
            + " entries which does not match"
            + " the number of column headers "
            + str(len(columnHeadersList))
            + "!",
        )


def stripWhiltespaceFromStrList(strListInOut):
    for i in range(len(strListInOut)):
        strListInOut[i] = strListInOut[i].strip()


g_expectedBuildsCsvFileHeadersRequired = ("group", "site", "buildname")


def getExpectedBuildsListOfDictsfromCsvFile(expectedBuildsFileName):
    return readCsvFileIntoListOfDicts(
        expectedBuildsFileName, g_expectedBuildsCsvFileHeadersRequired,
    )
...
def getTestsWtihIssueTrackersListFromCsvFile(testsWithIssueTrackersFile):
    return readCsvFileIntoListOfDicts(
        testsWithIssueTrackersFile, g_testsWithIssueTrackersCsvFileHeadersRequired,
    )
...
#
def writeTestsListOfDictsToCsvFileStructure(
    testsLOD,
    issueTrackerUrl="",
    issueTracker="",
):
...
def getAndCacheCDashQueryDataOrReadFromCache(
    ...
    if (
        ...
        with open(cdashQueryDataCacheFile) as cacheFile:
...
def getCDashIndexQueryUrl(cdashUrl, projectName, date, filterFields):
    ...
    (projectName,) = normalizeUrlStrings(
        projectName,
    )
    if date:
        dateArg = "&date=" + date
...
def getCDashIndexBrowserUrl(cdashUrl, projectName, date, filterFields):
    ...
    if date:
        ...
        dateArg = ""
    return cdashUrl + "/index.php?project=" + projectName + dateArg + "&" + filterFields
...
#
def createLookupDictForListOfDicts(
    listOfDicts,
    listOfKeys,
    ...
    idx = 0
    numRemoved = 0
    duplicateIndexesToRemoveFromList = []
    for dictEle in listOfDicts:
        ...
        if currentLookupDictRef:
            ...
            if hasSameKeyValuePairs and removeExactDuplicateElements:
                ...
                raiseDuplicateDictEleException(
                    idx,
                    dictEle,
                    listOfKeys,
                    lookedUpIdx,
...
def raiseDuplicateDictEleException(
    idx,
    dictEle,
    listOfKeys,
    lookedUpIdx,
...
def lookupDictGivenLookupDict(
    ...
    if len(listOfKeys) != len(listOfValues):
        raise Exception(
            "Error, len(listOfKeys)="
            + str(len(listOfKeys))
            + " != len(listOfValues)="
            + str(len(listOfValues))
            + " where"
            + " listOfKeys="
            + str(listOfKeys)
            + " and listOfValues="
...
class SearchableListOfDicts:
    ...
    def __str__(self):
        myStr = (
            "SearchableListOfDicts{listOfDicts="
            + str(self.__listOfDicts)
            + ", listOfKeys="
            + str(self.__listOfKeys)
            + ", lookupDict="
            + str(self.__lookupDict)
            + "}"
        )
        return myStr
    ...
    def lookupDictGivenKeyValueDict(self, keyValueDictToFind, alsoReturnIdx=False):
        ...
        lookupRtn = self.lookupDictGivenKeyValuesList(
    ...
    def lookupDictGivenKeyValuesList(self, keyValuesListToFind, alsoReturnIdx=False):
        lookupRtn = lookupDictGivenLookupDict(
            self.__lookupDict, self.__listOfKeys, keyValuesListToFind, alsoReturnIdx,
        )
        return lookupRtn
...
# => "testname" : test_dict.
def createSearchableListOfTests(
    testsListOfDicts,
    removeExactDuplicateElements=False,
    checkDictsAreSame_in=checkDictsAreSame,
):
    return SearchableListOfDicts(
        testsListOfDicts,
...
def createTestToBuildSearchableListOfDicts(
    buildsLOD,
    removeExactDuplicateElements=False,
):
    return SearchableListOfDicts(
...
class MatchDictKeysValuesFunctor:
...
class AddIssueTrackerInfoToTestDictFunctor:
    ...
    def __call__(self, testDict_inout):
        ...
        matchingDict = self.__testsWithIssueTrackersSLOD.lookupDictGivenKeyValueDict(
            testDict_inout,
        )
        if matchingDict:
            issue_tracker = matchingDict["issue_tracker"]
            issue_tracker_url = matchingDict["issue_tracker_url"]
        elif self.__addEmptyOnNoMatch:
            issue_tracker = ""
            issue_tracker_url = ""
        else:
            raise Exception(
                "Error, testDict_inout="
                + str(testDict_inout)
                + " does not have an assigned issue tracker!",
            )
        testDict_inout["issue_tracker"] = issue_tracker
        testDict_inout["issue_tracker_url"] = issue_tracker_url
...
#
def splitTestsOnMatchExpectedBuilds(
    testsWithIssueTrackersLOD,
    testToExpectedBuildsSLOD,
):
    return splitListOnMatch(
        testsWithIssueTrackersLOD, MatchDictKeysValuesFunctor(testToExpectedBuildsSLOD),
    )
...
#
def doTestsWithIssueTrackersMatchExpectedBuilds(
    testsWithIssueTrackersLOD,
    testToExpectedBuildsSLOD,
):
    # Get list of tests matching and non-matching expected builds (ignoring
    # matching tests list)
    (_, nomatchingTestsLOD) = splitTestsOnMatchExpectedBuilds(
...
#
def sortTestHistoryGetStatistics(
    testHistoryLOD,
    currentTestDate,
    testingDayStartTimeUtc,
    daysOfHistory,
):
    # Helper functions
    def incr(testDict, key):
        testDict[key] = testDict[key] + 1

    def decr(testDict, key):
        testDict[key] = testDict[key] - 1
    ...
    testingDayTimeObj = CBTD.CDashProjectTestingDay(
    ...
    if testStatus == "Missing":
        ...
        testHistoryStats["consec_missing_days"] = (
            currentTestDateDT - topTestDictTestingDayDT
        ).days
    ...
    if testStatus == "Passed":
        previousTestStatusPassed = True
    else:
        previousTestStatusPassed = False
    ...
    for pastTestDict_k in sortedTestHistoryLOD:
        ...
        if (
            pastTestStatus_k == "Passed"
        ) == previousTestStatusPassed and not initialTestStatusHasChanged:
...
def getUniqueSortedTestsHistoryListOfDicts(inputSortedTestHistoryLOD):
    ...
    while idx < len(inputSortedTestHistoryLOD):
        ...
        if not checkCDashTestDictsAreSame(
            candidateTestDict, "a", lastUniqueTestDict, "b",
        )[0]:
...
# the dicts are different for a single key/value pair.
#
def checkCDashTestDictsAreSame(
    testDict_1,
    testDict_1_name,
    testDict_2,
    testDict_2_name,
    ...
    if ("testDetailsLink" in testDict_1.keys()) and (
        "testDetailsLink" in testDict_2.keys()
    ):
        (test1d_1, buildid_1) = extractTestIdAndBuildIdFromTestDetailsLink(
            testDict_1["testDetailsLink"],
        )
        (test1d_2, buildid_2) = extractTestIdAndBuildIdFromTestDetailsLink(
            testDict_2["testDetailsLink"],
        )
    ...
    return checkDictsAreSame(
        testDict_1_copy, testDict_1_name, testDict_2_copy, testDict_2_name,
    )
...
#
class AddTestHistoryToTestDictFunctor:
    ...
    def __call__(self, testDict):
        ...
        beginEndUrlFields = (
            "begin=" + dateRangeBeginDateStr + "&end=" + dateRangeEndDateStr
        )
        ...
        testHistoryQueryFilters = (
            ...
            + "&field1=buildname&compare1=61&value1="
            + buildName_url
            + "&field2=testname&compare2=61&value2="
            + testname_url
            + "&field3=site&compare3=61&value3="
            + site_url
        )

        # URL used to get the history of the test in JSON form
        testHistoryQueryUrl = getCDashQueryTestsQueryUrl(
            cdashUrl, projectName_url, None, testHistoryQueryFilters,
        )

        # URL to embed in email to show the history of the test to humans
        testHistoryBrowserUrl = getCDashQueryTestsBrowserUrl(
            cdashUrl, projectName_url, None, testHistoryQueryFilters,
        )
        ...
        buildHistoryEmailUrl = getCDashIndexBrowserUrl(
            ...
            + "&field1=buildname&compare1=61&value1="
            + buildName_url
            + "&field2=site&compare2=61&value2="
            + site_url,
        )
        ...
        testHistoryCacheFileFullName = getTestHistoryCacheFileName(
            self.__date, site, buildName, testname, daysOfHistory,
        )
        ...
        testHistoryCacheFilePath = (
            ...
            + getCompressedFileNameIfTooLong(
                testHistoryCacheFileFullName, self.__date + "-", "json",
            )
        )
        ...
        if testStatus == "Missing":
            ...
            if self.__requireMatchTestTopTestHistory:
                if testDict.get("status", None) != testStatus:
                    raise Exception(
                        "Error, test testDict['status'] = '"
                        + str(testDict.get("status", None))
                        + "'"
                        + " != "
                        + "top test history testStatus = '"
                        + testStatus
                        + "'"
                        + " where:\n\n"
                        + "   testDict = "
                        + sorted_dict_str(testDict)
                        + "\n\n"
                        + "   top test history dict = "
                        + sorted_dict_str(testHistoryLOD[0])
                        + "\n\n",
                    )
                if (
                    testDict.get("buildstarttime", None)
        ...
        if testDict.get("status", None) == None:
            ...
            pp.pprint(testDict)
            raise Exception(
                "Error, testDict['status']==None for testDict=" + str(testDict),
            )
...
#
class AddCDashTestingDayFunctor:
...
def getMissingExpectedBuildsList(buildsSearchableListOfDicts, expectedBuildsList):
    ...
    for expectedBuildDict in expectedBuildsList:
        ...
        buildSummaryDict = buildsSearchableListOfDicts.lookupDictGivenKeyValueDict(
            expectedBuildDict,
        )
...
def downloadBuildsOffCDashAndFlatten(
    ...
    buildsListOfDicts = flattenCDashIndexBuildsToListOfDicts(fullCDashIndexBuildsJson)
...
def downloadTestsOffCDashQueryTestsAndFlatten(
    ...
    fullCDashQueryTestsJson = getAndCacheCDashQueryDataOrReadFromCache(
        cdashQueryTestsUrl,
        fullCDashQueryTestsJsonCacheFile,
        useCachedCDashData,
        alwaysUseCacheFileIfExists,
        verbose=verbose,
        extractCDashApiQueryData_in=extractCDashApiQueryData_in,
    ...
    testsListOfDicts = flattenCDashQueryTestsToListOfDicts(fullCDashQueryTestsJson)
    return testsListOfDicts
...
#
def sortAndLimitListOfDicts(listOfDicts, sortKeyList=None, limitRowsToDisplay=None):
    # Sort the list
    if sortKeyList:
...
def getOverallCDashReportSummaryLine(cdashReportData, buildsetName, date):
    ...
    if cdashReportData.summaryLineDataNumbersList:
        summaryLine += (
            " (" + ", ".join(cdashReportData.summaryLineDataNumbersList) + ")"
        )
...
def getFullCDashHtmlReportPageStr(
    cdashReportData,
    pageTitle="",
    pageStyle="",
    ...
    if pageTitle:
        htmlPage += "<h2>" + pageTitle + "</h2>\n\n"

    htmlPage += cdashReportData.htmlEmailBodyTop + "\n"
    ...
    htmlPage += cdashReportData.htmlEmailBodyBottom + "\n"
...
class TableColumnData:
    ...
    def __init__(self, colHeader, dictKey, colAlign="left"):
        ...
        if colAlign not in self.validColAlignList:
            raise Exception(
                "Error, colAlign="
                + colAlign
                + " not valid.  Please choose from"
                + " the list ['"
                + "', '".join(validColAlignList)
                + "']!",
            )
...
def colorHtmlText(htmlText, color_in):
    if color_in == None or color_in == "":
        return htmlText
    if color_in == "red" or color_in == "green" or color_in == "gray" or color_in == "orange":
        None  # Okay!
    else:
        raise Exception(
            "Error, color='"
...
#
def createHtmlTableStr(
    tableTitle, colDataList, rowDataList, htmlStyle=None, htmlTableStyle=None,
):
    htmlStr = ""
    ...
    if htmlStyle == "":
        ...
        htmlStyleUsed = htmlStyle
    else:
        htmlStyleUsed = defaultHtmlStyle
    if htmlStyleUsed:
        htmlStr += htmlStyleUsed + "\n"

    # Table title and <table style=...>
    htmlStr += "<h3>" + tableTitle + "</h3>\n"
    if htmlTableStyle != None:
        htmlTableStyleUsed = htmlTableStyle
    else:
        htmlTableStyleUsed = 'style="width:100%" boarder="1"'
        ...
        for colData in colDataList:
            ...
            if entry == None:
                raise Exception(
                    ...
                    + " row "
                    + str(row_i)
                    + " entry is 'None' which is not allowed!\n\n"
                    + "Row dict = "
                    + str(rowData),
                )
            ...
            htmlStr += '<td align="' + colData.colAlign + '">' + entryStr + "</td>\n"
            col_j += 1
        htmlStr += "</tr>\n\n"
        row_i += 1
...
#
def getCDashDataSummaryHtmlTableTitleStr(
    dataTitle,
    dataCountAcronym,
    numItems,
...
#
def createCDashDataSummaryHtmlTableStr(
    dataTitle,
    dataCountAcronym,
    colDataList,
    rowDataList,
    ...
    return createHtmlTableStr(
        tableTitle, colDataList, rowDataListDisplayed, htmlStyle, htmlTableStyle,
    )
...
def createCDashTestHtmlTableStr(
    ...
    tableTitle = colorHtmlText(
        getCDashDataSummaryHtmlTableTitleStr(
            testsetTypeInfo.testsetDescr,
            testsetTypeInfo.testsetAcro,
            testTypeCountNum,
            limitRowsToDisplay,
        ),
    ...
    testsColDataList = [
        ...
        consecCol,
        tcd(
            "Non-pass Last " + str(daysOfHistory) + " Days",
            "nopass_last_x_days",
            "right",
        ),
...
#
class IssueTrackerTestsStatusReporter:
    # Constructor
    #
    ...
    def __init__(self, verbose=True):
        ...
        self.testsetsReporter = TestsetsReporter(
            self.cdashReportData, htmlStyle="", verbose=verbose,
        )
    ...
    def getIssueTrackerTestsStatusReport(self):
        if self.issueTracker == None:
            return None
        testsSummaryTitle = (
            "Test results for issue "
            + self.issueTracker
            + " as of "
            + self.cdashTestingDay
        )
        return self.testsetsReporter.getTestsHtmlReportStr(
            testsSummaryTitle, detailsBlockSummary="Detailed test results",
        )
...
def getIssueTrackerFieldsAndAssertAllSame(testsLOD):
    ...
    i = 0
    for testDict in testsLOD:
        (issue_tracker_i, issue_tracker_url_i) = getIssueTrackerFieldsFromTestDict(
            testDict, i,
        )
        if issue_tracker == None and issue_tracker_url == None:
            ...
            assertSameIssueTrackerUrl(
                issue_tracker_url_i, issue_tracker_url, testDict, i,
            )
        i += 1
...
def assertSameIssueTracker(issue_tracker, issue_tracker_expected, testDict, idx):
    if issue_tracker != issue_tracker_expected:
        raise IssueTrackerFieldError(
            ...
            + " at index "
            + str(idx)
            + " has a different 'issue_tracker' field '"
            + str(issue_tracker)
            + "' than the"
            + " expected value of '"
            + str(issue_tracker_expected)
            + "'!",
        )


def assertSameIssueTrackerUrl(
    issue_tracker_url,
    issue_tracker_url_expected,
    testDict,
    idx,
):
    if issue_tracker_url != issue_tracker_url_expected:
        raise IssueTrackerFieldError(
            ...
            + " at index "
            + str(idx)
            + " has a different 'issue_tracker_url' field '"
            + str(issue_tracker_url)
            + "' than the"
            + " expected value of '"
            + str(issue_tracker_url_expected)
            + "'!",
        )
...
class SingleBuildsetReporter:
    ...
    def reportSingleBuildset(
        ...
        buildsetGlobalPass,
        buildsetColor,
        buildsetColDataList=None,
        verbose=True,
    ):
        buildsetNum = len(buildsetLOD)

        buildsetSummaryStr = getCDashDataSummaryHtmlTableTitleStr(
            buildsetDescr, buildsetAcro, buildsetNum,
        )
...
class SingleTestsetReporter:
    ...
    def reportSingleTestset(
        ...
        testsetSummaryStr = getCDashDataSummaryHtmlTableTitleStr(
            testsetTypeInfo.testsetDescr, testsetTypeInfo.testsetAcro, testsetTotalSize,
        )
        ...
        if testsetTotalSize > 0:
            ...
            self.cdashReportData.htmlEmailBodyTop += (
                colorHtmlText(testsetSummaryStr, testsetTypeInfo.testsetColor)
                + "<br>\n"
            )
            ...
            self.cdashReportData.htmlEmailBodyBottom += createCDashTestHtmlTableStr(
                ...
                htmlTableStyle=self.htmlTableStyle,
            )
...
class TestsetsReporter:
    ...
    def __init__(
        self,
        cdashReportData,
        testsetAcroList=getStandardTestsetAcroList(),
        htmlStyle=None,
        htmlTableStyle=None,
        verbose=True,
    ):
        ...
        self.singleTestsetReporter = SingleTestsetReporter(
            cdashReportData,
            htmlStyle=htmlStyle,
            htmlTableStyle=htmlTableStyle,
            verbose=verbose,
    ...
    def reportTestsets(self, testsLOD):
        ...
        for testsetAcro in self.testsetAcroList:
            ...
            if testsetLOD:
                ...
                self.singleTestsetReporter.reportSingleTestset(
                    testsetTypeInfo, len(testsetLOD), testsetLOD,
                )
...
def binTestDictsByIssueTracker(testsLOD):
    ...
    for testDict in testsLOD:
        issueTracker = testDict.get("issue_tracker", None)
        if issueTracker:
            issueTrackerBinTestsLOD = testDictsByIssueTracker.setdefault(
                issueTracker, [],
            )
...
#
def createHtmlMimeEmail(
    fromAddress, toAddress, subject, textBody, htmlBody, doRemoveSoftHyphens=False,
):
