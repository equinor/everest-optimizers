...
def injectCmndLineOptionsInParser(clp, gitoliteRootDefault=""):
    ...
    clp.add_option(
        "--cdash-project-testing-day-start-time",
        dest="cdashProjectTestingDayStartTime",
        type="string",
        default="00:00",
    ...
    clp.add_option(
        "--cdash-nonpassed-tests-filters",
        dest="cdashNonpassedTestsFilters",
        type="string",
        default="",
    ...
    clp.add_option(
        ...
        dest="testsWithIssueTrackersFile",
        type="string",
        default="",
    ...
    clp.add_option(
        "--cdash-queries-cache-dir",
        dest="cdashQueriesCacheDir",
        type="string",
        default=cdashQueriesCacheDir_default,
        help="Cache CDash query data this directory."
         " [default = '"
        + cdashQueriesCacheDir_default
        + "']",
    )

    clp.add_option(
        "--cdash-base-cache-files-prefix",
        dest="cdashBaseCacheFilesPrefix",
        type="string",
        default="",
    ...
    clp.add_option(
        ...
        dest="writeUnexpectedBuildsToFile",
        type="string",
        default="",
    ...
    clp.add_option(
        ...
        dest="writeFailingTestsWithoutIssueTrackersToFile",
        type="string",
        default="",
...
def validateAndConvertCmndLineOptions(inOptions):
    if inOptions.date == "":
        print(
            "Error, can't have empty --date, must pass in --date=YYYY-MM-DD"
             " or special values --date=today or --date=yesterday!",
        )
        sys.exit(1)
    else:
        dateTimeObj = CDQAR.convertInputDateArgToYYYYMMDD(
            inOptions.cdashProjectTestingDayStartTime, inOptions.date,
        )
...
def setExtraCmndLineOptionsAfterParse(inOptions_inout):
    inOptions_inout.filterOutBuildsAndTestsNotMatchingExpectedBuilds = inOptions_inout.filterOutBuildsAndTestsNotMatchingExpectedBuildsStr == "on"

    inOptions_inout.useCachedCDashData = inOptions_inout.useCachedCDashDataStr == "on"

    inOptions_inout.requireTestHistoryMatchNonpassingTests = inOptions_inout.requireTestHistoryMatchNonpassingTestsStr == "on"

    inOptions_inout.printDetails = inOptions_inout.printDetailsStr == "on"

    inOptions_inout.listUnexpectedBuilds = inOptions_inout.listUnexpectedBuildsStr == "on"
    ...
    inOptions_inout.emailWithoutSoftHyphens = inOptions_inout.emailWithoutSoftHyphensStr == "on"
...
def fwdCmndLineOptions(inOptions, lt=""):
    ...
    cmndLineOpts = (
        ...
        + lt
        + "  --cdash-project-testing-day-start-time='"
        + io.cdashProjectTestingDayStartTime
        + "'"
        + lt
        + "  --cdash-project-name='"
        + io.cdashProjectName
        ...
        + lt
        + "  --cdash-builds-filters='"
        + io.cdashBuildsFilters
        + "'"
        + lt
        + "  --cdash-nonpassed-tests-filters='"
        + io.cdashNonpassedTestsFilters
        + "'"
        + lt
        + "  --expected-builds-file='"
        + io.expectedBuildsFile
        + "'"
        + lt
        + "  --tests-with-issue-trackers-file='"
        + io.testsWithIssueTrackersFile
        + "'"
        + lt
        + "  --filter-out-builds-and-tests-not-matching-expected-builds='"
        + io.filterOutBuildsAndTestsNotMatchingExpectedBuildsStr
        + "'"
        + lt
        + "  --cdash-queries-cache-dir='"
        + io.cdashQueriesCacheDir
        + "'"
        + lt
        + "  --cdash-base-cache-files-prefix='"
        + io.cdashBaseCacheFilesPrefix
        + "'"
        + lt
        + "  --use-cached-cdash-data='"
        + io.useCachedCDashDataStr
        + "'"
        + lt
        + "  --limit-test-history-days='"
        + str(io.testHistoryDays)
        + "'"
        + lt
        + "  --limit-table-rows='"
        + str(io.limitTableRows)
        + "'"
        + lt
        + "  --require-test-history-match-nonpassing-tests='"
        + io.requireTestHistoryMatchNonpassingTestsStr
        + "'"
        + lt
        + "  --print-details='"
        + io.printDetailsStr
        ...
        + lt
        + "  --list-unexpected-builds='"
        + io.listUnexpectedBuildsStr
        + "'"
        + lt
        + "  --write-unexpected-builds-to-fileo='"
        + io.writeUnexpectedBuildsToFile
        + "'"
        + lt
        + "  --write-failing-tests-without-issue-trackers-to-file='"
        + io.writeFailingTestsWithoutIssueTrackersToFile
        + "'"
        + lt
        + "  --write-test-data-to-file='"
        + io.writeTestDataToFile
        + "'"
        + lt
        + "  --write-email-to-file='"
        + io.writeEmailToFile
...
if __name__ == "__main__":
    ...
    print("***")
    print(
        "*** Query and analyze CDash results for "
        + inOptions.buildSetName
        + " for testing day "
        + inOptions.date,
    )
    print("***")
    ...
    try:
        ...
        testsToExpectedBuildsSLOD = CDQAR.createTestToBuildSearchableListOfDicts(
            expectedBuildsLOD,
        )
        ...
        if inOptions.testsWithIssueTrackersFile:
            fullTestsWithIssueTrackersLOD = (
                CDQAR.getTestsWtihIssueTrackersListFromCsvFile(
        ...
        if inOptions.filterOutBuildsAndTestsNotMatchingExpectedBuilds:
            (testsWithIssueTrackersLOD, testsWithIssueTrackersNotExpectedLOD) = (
                CDQAR.splitTestsOnMatchExpectedBuilds(
                    fullTestsWithIssueTrackersLOD, testsToExpectedBuildsSLOD,
                )
            )
            print(
                "Num tests with issue trackers matching expected builds = "
                + str(len(testsWithIssueTrackersLOD)),
            )
        else:
            testsWithIssueTrackersLOD = fullTestsWithIssueTrackersLOD
        print("Num tests with issue trackers = " + str(len(testsWithIssueTrackersLOD)))
        ...
        # key/value pairs.
        testsWithIssueTrackersSLOD = CDQAR.createSearchableListOfTests(
        ...
        testsWithIssueTrackerMatchFunctor = CDQAR.MatchDictKeysValuesFunctor(
        ...
        if inOptions.filterOutBuildsAndTestsNotMatchingExpectedBuilds:
            print(
                "Num builds matching expected builds = " + str(len(buildsExpectedLOD)),
            )
        ...
        if inOptions.filterOutBuildsAndTestsNotMatchingExpectedBuilds:
            (nonpassingTestsLOD, nonpassingTestsNotExpectedLOD) = (
                CDQAR.splitTestsOnMatchExpectedBuilds(
                    fullNonpassingTestsLOD, testsToExpectedBuildsSLOD,
                )
            )
            print(
                "Num nonpassing tests matching expected builds = "
                + str(len(nonpassingTestsLOD)),
            )
        else:
            nonpassingTestsLOD = fullNonpassingTestsLOD

        print("Num nonpassing tests = " + str(len(nonpassingTestsLOD)))
        ...
        cdashReportData.htmlEmailBodyTop += (
            ...
            + cdashNonpassingTestsBrowserUrl
            + '">'
            + "Non-passing Tests on CDash</a> (num="
            + str(len(nonpassingTestsLOD))
            + ")<br>\n"
        )
        ...
        (
            ...
            nonpassingTestsWithoutIssueTrackersLOD,
        ) = CDQAR.splitListOnMatch(
            nonpassingTestsLOD, testsWithIssueTrackerMatchFunctor,
        )
        print(
            "Num nonpassing tests without issue trackers = "
            + str(len(nonpassingTestsWithoutIssueTrackersLOD)),
        )
        print(
            "Num nonpassing tests with issue trackers = "
        ...
        print(
            "Num nonpassing tests without issue trackers Failed = " + str(len(twoifLOD)),
        )
        print(
            "Num nonpassing tests without issue trackers Not Run = "
            + str(len(twoinrLOD)),
        )
        ...
        print(
            "Num nonpassing tests with issue trackers Not Run = " + str(len(twinrLOD)),
        )
        ...
        print(
            "Num tests with issue trackers gross passing or missing = "
        ...
        buildsetReporter.reportSingleBuildset(
            "Builds Missing",
            "bm",
        ...
        buildsWithConfigureFailuresLOD = CDQAR.getFilteredList(
            buildsSLOD, CDQAR.buildHasConfigureFailures,
        )

        buildsetReporter.reportSingleBuildset(
            "Builds with Configure Failures",
            "cf",
        ...
        buildsWithBuildFailuresLOD = CDQAR.getFilteredList(
            buildsSLOD, CDQAR.buildHasBuildFailures,
        )

        buildsetReporter.reportSingleBuildset(
            "Builds with Build Failures",
            "bf",
        ...
        testsetReporter = CDQAR.SingleTestsetReporter(
            cdashReportData, addTestHistoryStrategy=addTestHistoryStrategy,
        )
        ...
        if inOptions.writeUnexpectedBuildsToFile:
            ...
            CDQAR.writeExpectedBuildsListOfDictsToCsvFile(
                buildsUnexpectedLOD, unexpectedBuildsCsvFileName,
            )
        ...
        if inOptions.writeTestDataToFile:
            ...
            CDQAR.foreachTransform(
                testDataLOD, CDQAR.AddCDashTestingDayFunctor(inOptions.date),
            )
    ...
    summaryLine = CDQAR.getOverallCDashReportSummaryLine(
        cdashReportData, inOptions.buildSetName, inOptions.date,
    )
    ...
    if inOptions.writeEmailToFile:
        ...
        fullCDashHtmlReportPageStr = CDQAR.getFullCDashHtmlReportPageStr(
            cdashReportData, pageTitle=summaryLine, pageStyle=defaultPageStyle,
        )
        with open(inOptions.writeEmailToFile, "w") as outFile:
            outFile.write(fullCDashHtmlReportPageStr)

    if inOptions.sendEmailTo:
        htmlEmailBodyStr = CDQAR.getFullCDashHtmlReportPageStr(
            cdashReportData, pageStyle=defaultPageStyle,
        )
        for emailAddress in inOptions.sendEmailTo.split(","):
            emailAddress = emailAddress.strip()
            print("\nSending email to '" + emailAddress + "' ...")
            msg = CDQAR.createHtmlMimeEmail(
                inOptions.emailFromAddress,
                emailAddress,
                summaryLine,
                "",
                htmlEmailBodyStr,
                inOptions.emailWithoutSoftHyphens,
            )
