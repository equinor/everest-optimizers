...
from TribitsDependencies import (
    getProjectDependenciesFromXmlFile,
)
...
def performAnyBuildTestActions(inOptions):
    if (
        inOptions.doConfigure
        or inOptions.doBuild
        or inOptions.doTest
        or inOptions.doAll
        or inOptions.localDoAll
    ):
        return True
    return False
...
def assertGitRepoExists(inOptions, gitRepo):
    ...
    if not os.path.os.path.exists(gitRepoDir):
        raise Exception(
            "Error, the specified git repo '" + gitRepo.repoName + "' directory"
        ...
        if getDefaultProjectDependenices().packageNameToID(packageName) == -1:
            ...
            for i in range(getDefaultProjectDependenices().numPackages()):
                if validPackagesListStr != "":
                    validPackagesListStr += ", "
                validPackagesListStr += (
                    getDefaultProjectDependenices().getPackageByID(i).packageName
                )
            raise Exception(
                ...
                + "="
                + packagesListStr
                + ".  The valid package names include: "
                + validPackagesListStr,
            )
...
def assertExtraBuildConfigFiles(extraBuilds):
    ...
    for extraBuild in extraBuilds.split(","):
        ...
        if not os.path.exists(extraBuildConfigFile):
            raise Exception(
                "Error, the extra build configuration file "
                + extraBuildConfigFile
                + " does not exit!",
            )
...
def getRepoStats(inOptions, gitRepo_inout):
    ...
    try:
        os.chdir(gitRepoDir)
        gitRepo_inout.gitRepoStats = gitdist.getRepoStats(
            gitdistOptions, getCmndOutputForGitDist,
        )
    finally:
        os.chdir(pwd)
...
def getReposStats(inOptions, tribitsGitRepos):
    ...
    for gitRepo in tribitsGitRepos.gitRepoList():
        ...
        repoStatTable.insertRepoStat(
            repoStatTableDirName, gitRepo.gitRepoStats, repoIdx,
        )
        repoIdx += 1
...
def assertRepoHasBranchAndTrackingBranch(inOptions, gitRepo):
    ...
    if gitRepoStats.branch == "HEAD":
        raise Exception(
            "Error, the " + repoNameEntry + " is in a detached head state which"
            " is not allowed in this case!",
        )
    if gitRepoStats.trackingBranch == "":
        raise Exception(
            "Error, the " + repoNameEntry + " is not on a tracking branch which"
            " is not allowed in this case!",
        )
...
def executePull(
    gitRepo, inOptions, baseTestDir, outFile, pullFromRepo=None, doRebase=False,
):
    cmnd = inOptions.git + " pull"
    if pullFromRepo:
        repoSpaceBranch = pullFromRepo.remoteRepo + " " + pullFromRepo.remoteBranch
        print(
            "\nPulling in updates to local repo '"
            + gitRepo.repoName
            + "'"
            + " from '"
            + repoSpaceBranch
    ...
    if doRebase:
        cmnd += (
            " && " + inOptions.git + " rebase " + gitRepo.gitRepoStats.trackingBranch
        )
    outFileFullPath = os.path.join(baseTestDir, outFile)
    (pullRtn, pullTimings) = echoRunSysCmnd(
        cmnd,
...
class GitRepo:
    def __init__(
        ...
        if (self.repoName and self.repoHasPackages) and (self.repoName != self.repoDir):
            raise Exception(
                "ERROR!  For extra repo '"
                + repoName
                + "', if repoHasPackages==True"
                + " then repoDir must be same as repo name, not '"
                + repoDir
                + "'!",
            )
        if self.repoType != "GIT":
            raise Exception(
                ...
                + " '"
                + self.repoType
                + "' is not supported by the checkin-test.py script, only 'GIT'!",
            )
    ...
    def __str__(self):
        return (
            ...
            + "'"
            + ", repoHasPackages="
            + str(self.repoHasPackages)
            + ", repoPrePost="
            + str(self.repoPrePost)
            + ", hasChanges="
            + str(self.hasChanges)
            + "}"
        )
...
def getExtraReposPyFileFromCmakeFile(
    inOptions, extraReposPythonOutFile, consoleOutputFile=None, verbose=False,
):
    ...
    cmnd = (
        '"' + inOptions.withCmake + '"' + " -DSUPPRESS_PRINT_VAR_OUTPUT=TRUE"
        " -DPROJECT_SOURCE_DIR="
        + inOptions.srcDir
        + " -DTRIBITS_BASE_DIR="
        + inOptions.tribitsDir
        + " -DEXTRA_REPOS_FILE="
        + extraReposFile
        + " -DENABLE_KNOWN_EXTERNAL_REPOS_TYPE="
        + inOptions.extraReposType
        + " -DEXTRA_REPOS="
        + inOptions.extraRepos
        + " -DEXTRA_REPOS_PYTHON_OUT_FILE="
        + extraReposPythonOutFile
    )
...
class TribitsGitRepos:
    ...
    def initFromCommandlineArguments(
        self, inOptions, consoleOutputFile=None, verbose=True,
    ):
        self.reset()
        ...
        if inOptions.extraRepos != "" and (
            ...
            extraReposPythonOutFile = getProjectExtraReposPythonOutFile(
                inOptions.projectName,
            )
            getExtraReposPyFileFromCmakeFile(
                inOptions,
                extraReposPythonOutFile,
    ...
    def __str__(self):
        ...
        strRep += (
            "  tribitsPreRepoNamesList = " + str(self.__tribitsPreRepoNamesList) + "\n"
        )
        strRep += (
            "  tribitsExtraRepoNamesList = "
            + str(self.__tribitsExtraRepoNamesList)
            + "\n"
        )
        strRep += (
            "  tribitsAllExtraRepoNamesList = "
            + str(self.__tribitsAllExtraRepoNamesList)
            + "\n"
        )
        strRep += "  }\n"
        return strRep
...
def createAndGetProjectDependencies(inOptions, baseTestDir, tribitsGitRepos):
    ...
    projectDepsXmlFile = (
        baseTestDir + "/" + getProjectDependenciesXmlFileName(inOptions.projectName)
    )
    if not inOptions.skipDepsUpdate:
        ...
        cmakeArgumentList = [
            ...
            cmakeScopedDefine(
                inOptions.projectName, "TRIBITS_DIR", inOptions.tribitsDir,
            ),
            ...
            cmakeScopedDefine(
                inOptions.projectName, "DEPS_XML_OUTPUT_FILE", projectDepsXmlFile,
            ),
...
class RemoteRepoAndBranch:
    ...
    def __str__(self):
        return (
            "RemoteRepoAndBranch{repoRepo='"
            + str(self.remoteRepo)
            + "'"
            + ", remoteBranch='"
            + str(self.remoteBranch)
            + "'"
            + "}"
        )
...
def matchExtraRepoLocalRepoMatchLocalRepo(repoName, extraRepoLocalRepoName):
    if repoName == extraRepoLocalRepoName or (repoName == "" and extraRepoLocalRepoName == "BASE_REPO"):
        return True
    return False
...
def parseExtraPullFromArgs(gitRepoList, extraPullFromArgs):
    ...
    for gitRepo in gitRepoList:
        repoExtraRemotePullsList.append(RepoExtraRemotePulls(gitRepo, []))
...
class BuildTestCase:
    def __init__(
        ...
        name,
        runBuildTestCase,
        validPackageTypesList,
        isDefaultBuild,
        skipCaseIfNoChangeFromDefaultEnables,
        extraCMakeOptions,
        buildIdx,
    ):
        self.name = name
...
def setBuildTestCaseInList(
    buildTestCaseList_inout,
    name,
    runBuildTestCase,
    validPackageTypesList,
    isDefaultBuild,
...
def readAndAppendCMakeOptions(
    projectName, fileName, cmakeOptions_inout, assertNoIllegalEnablesBool,
):
    success = True
    ...
    cmakeOptionsFile = open(fileName)
...
def getCurrentDiffOutputAndLogModified(inOptions, gitRepo, baseTestDir):
    ...
    if gitRepo.hasChanges:
        ...
        print(
            "\n  ==> '"
            + gitRepo.repoName
            + "': Does *not* have any modified "
            + "files!",
        )


def extractPackageEnablesFromChangeStatus(
    ...
    projectChangeLogic=DefaultProjectCiFileChangeLogic(),
):
    ...
    for modifiedFileFullPath in modifiedFilesList:
        ...
        if (
            gitRepo.repoName == ""
            and projectChangeLogic.isGlobalBuildFileRequiringGlobalRebuild(
                modifiedFileFullPath,
            )
        ):
        ...
        packageName = getPackageNameFromPath(
            projectDependenciesLocal, modifiedFileFullPath,
        )
...
def getTimeInMinFromTotalTimeLine(buildTestCaseName, totalTimeLine):
    if not totalTimeLine:
        return -1.0
    m = re.match(
        getTotalTimeBeginStr(buildTestCaseName) + r" = (.+) min", totalTimeLine,
    )
    if m and m.groups():
...
def analyzeResultsSendEmail(
    inOptions, buildTestCase, enabledPackagesList, cmakeOptions, startingTime, timings,
):
    ...
    if inOptions.doTest:
        ...
        if not testOutputExists:
            ...
            testResultsLine = getCmndOutput(
                "grep 'tests failed out of' " + getTestOutputFileName(), True, False,
            )
    ...
    if inOptions.doConfigure and not selectedFinalStatus:
        if configurePassed:
            ...
            buildCaseStatus += (
                "skipped configure, build, test due to no enabled packages"
            )
            overallPassed = True
            selectedFinalStatus = True
        elif configureOutputExists:
    ...
    emailBody += getStageStatus(
        "Configure", inOptions.doConfigure, configurePassed, timings.configure,
    )
    ...
    if (
        inOptions.sendEmailTo
        and buildTestCase.skippedConfigureDueToNoEnables
        and inOptions.abortGracefullyIfNoEnables
    ):
        print(
            buildTestCaseName
            + ": Skipping sending build/test case email "
        ...
        inOptions.sendEmailTo
        and inOptions.sendBuildCaseEmail == "only-on-failure"
        and overallPassed
    ):
        print(
            buildTestCaseName
        ...
        print(
            "\nSkipping sending final status email for "
            + buildTestCase.name
...
def getBuildTestCaseSummary(testCaseName, trimDown=True):
    ...
    if os.path.exists(absEmailBodyFileName):
        testCaseEmailStrArray = open(absEmailBodyFileName).readlines()
    else:
    ...
    if testCaseEmailStrArray:
        ...
        if trimDown:
            ...
            testSummaryLine = (
                summaryLineArray[0].strip() + ": " + summaryLineArray[2].strip()
            )
        else:
...
def getSummaryEmailSectionStr(inOptions, buildTestCaseList):
    ...
    for buildTestCase in buildTestCaseList:
        if (
            buildTestCase.runBuildTestCase
            and not buildTestCase.skippedConfigureDueToNoEnables
        ):
            summaryEmailSectionStr += getTestCaseEmailSummary(
                buildTestCase.name, buildTestCase.buildIdx,
            )
    return summaryEmailSectionStr
...
def getEnablesLists(
    ...
    if inOptions.enableAllPackages == "on":
        ...
        for gitRepo in gitRepoList:
            ...
            if os.path.exists(diffOutFileName):
                changedFileDiffOutputStr = open(diffOutFileName).read()
                # print("\nchangedFileDiffOutputStr:\n", changedFileDiffOutputStr)
                extractPackageEnablesFromChangeStatus(
                    changedFileDiffOutputStr,
                    inOptions,
                    gitRepo,
                    enablePackagesList,
                    verbose,
                    projectChangeLogic=projectChangeLogic,
                )
            elif verbose:
    ...
    if tribitsGitRepos.numTribitsAllExtraRepos() > 0:
        cmakePkgOptions.extend(
            [
                ...
                cmakeScopedDefine(
                    projectName,
                    "ENABLE_KNOWN_EXTERNAL_REPOS_TYPE",
                    inOptions.extraReposType,
                ),
                cmakeScopedDefine(
    ...
    for pkg in enablePackagesList:
        cmakePkgOptions.append(
            cmakeScopedDefine(projectName, "ENABLE_" + pkg + ":BOOL", "ON"),
        )

    cmakePkgOptions.append(
        cmakeScopedDefine(projectName, "ENABLE_ALL_OPTIONAL_PACKAGES:BOOL", "ON"),
    )

    if inOptions.enableAllPackages == "on":
        cmakePkgOptions.append(
            cmakeScopedDefine(projectName, "ENABLE_ALL_PACKAGES:BOOL", "ON"),
        )
    ...
    if inOptions.enableFwdPackages:
        ...
        cmakePkgOptions.append(
            cmakeScopedDefine(projectName, "ENABLE_ALL_FORWARD_DEP_PACKAGES:BOOL", "ON"),
        )
    else:
        cmakePkgOptions.append(
            cmakeScopedDefine(
                projectName, "ENABLE_ALL_FORWARD_DEP_PACKAGES:BOOL", "OFF",
            ),
        )
    ...
    if inOptions.disablePackages:
        ...
        for pkg in disablePackagesList:
            cmakePkgOptions.append(
                cmakeScopedDefine(projectName, "ENABLE_" + pkg + ":BOOL", "OFF"),
            )
...
def runBuildTestCase(inOptions, tribitsGitRepos, buildTestCase, timings):
    ...
    try:
        ...
        if inOptions.extraCmakeOptions:
            cmakeBaseOptions.extend(
                commandLineOptionsToList(inOptions.extraCmakeOptions),
            )
        cmakeBaseOptions.append(
            cmakeScopedDefine(projectName, "TRIBITS_DIR:PATH", inOptions.tribitsDir),
        )
        cmakeBaseOptions.append(
            cmakeScopedDefine(projectName, "ENABLE_TESTS:BOOL", "ON"),
        )
        cmakeBaseOptions.append(
            cmakeScopedDefine(
                projectName, "TEST_CATEGORIES:STRING", inOptions.testCategories,
            ),
        )
        cmakeBaseOptions.append(
            cmakeScopedDefine(projectName, "ALLOW_NO_PACKAGES:BOOL", "OFF"),
        )

        if inOptions.ctestTimeOut:
            cmakeBaseOptions.append(
                "-DDART_TESTING_TIMEOUT:STRING=" + str(inOptions.ctestTimeOut),
            )
        ...
        if preConfigurePassed:
            (cmakePkgOptions, enablePackagesList) = getEnablesLists(
                inOptions,
                buildTestCase.validPackageTypesList,
                buildTestCase.isDefaultBuild,
                buildTestCase.skipCaseIfNoChangeFromDefaultEnables,
                tribitsGitRepos,
                baseTestDir,
                True,
            )
        ...
        if preConfigurePassed:
            ...
            createConfigureFile(
                cmakeBaseOptions, "cmake", inOptions.srcDir, "do-configure.base",
            )

            print("\nCreating package-enabled configure file do-configure ...")
            createConfigureFile(
                cmakePkgOptions, "./do-configure.base", None, "do-configure",
            )
        ...
        if inOptions.doConfigure and not preConfigurePassed:
            ...
            (configureRtn, timings.configure) = echoRunSysCmnd(
                cmnd,
        ...
        if inOptions.doBuild and configurePassed:
            ...
            (buildRtn, timings.build) = echoRunSysCmnd(
                cmnd,
        ...
        if inOptions.doTest and buildPassed:
            ...
            (testRtn, timings.test) = echoRunSysCmnd(
                cmnd,
    ...
    print()
    print(
        "E) Analyze the overall results and send email notification ("
        + buildTestCaseName
        + ") ...",
    )
    print()
    ...
    if performAnyActions(inOptions):
        result = analyzeResultsSendEmail(
            inOptions,
            buildTestCase,
            enablePackagesList,
            cmakeOptions,
...
def cleanBuildTestCaseOutputFiles(
    runBuildTestCaseBool, inOptions, baseTestDir, buildTestCaseName,
):
    if runBuildTestCaseBool and not os.path.exists(buildTestCaseName):
        print(
            "\nSkipping cleaning build/test files for "
            + buildTestCaseName
...
def cleanBuildTestCaseSuccessFiles(
    runBuildTestCaseBool, inOptions, baseTestDir, buildTestCaseName,
):
...
def runBuildTestCaseDriver(
    inOptions, tribitsGitRepos, baseTestDir, buildTestCase, timings,
):
    success = True
    ...
    if buildTestCase.runBuildTestCase:
        try:
            ...
            result = runBuildTestCase(
                inOptions, tribitsGitRepos, buildTestCase, timings,
            )
            if not result:
                success = False
        except Exception:
...
def checkBuildTestCaseStatus(buildTestCase, inOptions):
    ...
    if not runBuildTestCaseBool:
        ...
        statusMsg = (
            "Test case "
            + buildTestCaseName
            + " was not run! => Does not affect push readiness!"
        )
        return (
            buildTestCaseActionsPass,
            buildTestCaseOkayToCommit,
            statusMsg,
            timeInMin,
        )
    ...
    if skippedConfigureDueToNoEnables:
        buildTestCaseActionsPass = True
        buildTestCaseOkayToCommit = True
        statusMsg = "Skipped configure, build, test due to no enabled packages! => Does not affect push readiness!"
        return (
            buildTestCaseActionsPass,
            buildTestCaseOkayToCommit,
            statusMsg,
    ...
    if not os.path.exists(buildTestCaseName) and not performAnyBuildTestActions(
        inOptions,
    ):
        buildTestCaseActionsPass = True
        buildTestCaseOkayToCommit = False
        statusMsg = (
            "No configure, build, or test for " + buildTestCaseName + " was requested!"
        )
        return (
            buildTestCaseActionsPass,
            buildTestCaseOkayToCommit,
    ...
    if os.path.exists(emailBodyFileName):
        timeInMinLine = getCmndOutput(
            "grep '"
            + getTotalTimeBeginStr(buildTestCaseName)
            + "' "
            + emailBodyFileName,
            True,
            False,
...
def getUserCommitMessageStr(inOptions):
    ...
    commitMsgHeaderFileStr = open(absCommitMsgHeaderFile).read()
...
def getLastCommitMessageStrFromRawCommitLogStr(rawLogOutput):
    ...
    if foundStatusHeader:
        ...
        if origLogStrList[-2] != "":
            raise Exception(
                "Error, there must be at least one blank line before the"
...
def getLocalCommitsSummariesStr(inOptions, gitRepo):
    ...
    localCommitsStr = "*** Commits for repo " + repoName + ":"
    if localCommitsExist:
...
def getLocalCommitsSHA1ListStr(inOptions, gitRepo):
    ...
    if len(rawLocalCommitsArray) > 1:
        return (
            "Other local commits for this build/test group: "
...
def checkinTest(tribitsDir, inOptions, configuration={}):
    ...
    inOptions.tribitsDir = tribitsDir

    ciSupportDir = os.path.join(tribitsDir, "ci_support")
    inOptions.ciSupportDir = ciSupportDir
    ...
    if inOptions.overallNumProcs:
        inOptions.makeOptions = (
            "-j" + inOptions.overallNumProcs + " " + inOptions.makeOptions
        )
        inOptions.ctestOptions = (
            "-j" + inOptions.overallNumProcs + " " + inOptions.ctestOptions
        )
    ...
    for buildname, buildopts in defaultBuilds:
        setBuildTestCaseInList(
            ...
            + [
                cmakeScopedDefine(
                    inOptions.projectName, "ENABLE_SECONDARY_TESTED_CODE:BOOL", "OFF",
                ),
            ]
            + buildopts,
        )
    ...
    if inOptions.stExtraBuilds:
        for ssExtraBuild in inOptions.stExtraBuilds.split(","):
            setBuildTestCaseInList(
                buildTestCaseList, ssExtraBuild, True, ["PT", "ST"], False, True, [],
            )
    ...
    try:
        ...
        if inOptions.enableAllPackages == "on":
            ...
            print(
                "git diffs w.r.t. tracking branch may be needed to look for "
                 "changed files?",
            )
            gitDiffsWrtTrackingBranchAreNeeded = True
        ...
        if doingAtLeastOnePull and pullPassed:
            ...
            for gitRepo in tribitsGitRepos.gitRepoList():
                ...
                if not reposAreClean:
                    ...
                    gitStatusOutput = getCmndOutput(
                        inOptions.git + " status",
                        True,
                        throwOnError=False,
                        workingDir=getGitRepoDir(inOptions.srcDir, gitRepo.repoDir),
                    )
        ...
        if doingAtLeastOnePull and pullPassed:
            ...
            if inOptions.extraPullFrom and pullPassed:
                repoExtraRemotePullsList = parseExtraPullFromArgs(
                    tribitsGitRepos.gitRepoList(), inOptions.extraPullFrom,
                )
                repoIdx = 0
                for repoExtraRemotePulls in repoExtraRemotePullsList:
                    gitRepo = repoExtraRemotePulls.gitRepo
                    remoteRepoAndBranchList = (
                        repoExtraRemotePulls.remoteRepoAndBranchList
                    )
        ...
        if not performAnyBuildTestActions(inOptions):
            ...
            if (
                reposAreClean
                and not pulledSomeChanges
                and inOptions.abortGracefullyIfNoChangesPulled
            ):
                print(
                ...
                reposAreClean
                and not hasChangesToPush
                and inOptions.abortGracefullyIfNoChangesToPush
            ):
                print(
        ...
        if inOptions.doPushReadinessCheck:
            ...
            (cmakePkgOptions, enabledPackagesList) = getEnablesLists(
                inOptions,
                allValidPackageTypesList,
                False,
                False,
            ...
            for i in range(len(buildTestCaseList)):
                ...
                buildTestCaseStatusStr = (
                    str(i) + ") " + buildTestCaseName + " => " + statusMsg
                )
            ...
            if (
                inOptions.doPushReadinessCheck
                and not okayToCommit
                and inOptions.forcePush
            ):
                forcedPushMsg = (
                    "\n***"
        ...
        if not inOptions.doPush:
            ...
            if not okayToPush:
                ...
                for gitRepo in tribitsGitRepos.gitRepoList():
                    ...
                    (pull2Rtn, pull2Time, pullGotChanges) = executePull(
                        gitRepo,
                        inOptions,
                        baseTestDir,
                        getFinalPullOutputFileName(gitRepo.repoName),
                        None,
                        doFinalRebase,
                    )
                ...
                for gitRepo in tribitsGitRepos.gitRepoList():
                    ...
                    try:
                        if gitRepo.gitRepoStats.numCommitsInt() > 0:
                            ...
                            lastCommitMessageStr = getLastCommitMessageStr(
                                inOptions, gitRepo,
                            )
                            localCommitSHA1ListStr = getLocalCommitsSHA1ListStr(
                                inOptions, gitRepo,
                            )

                            # Get then final commit message
                            finalCommitEmailBodyStr = lastCommitMessageStr
                            finalCommitEmailBodyStr += (
                                getAutomatedStatusSummaryHeaderStr()
                            )
                            ...
                            finalCommitEmailBodyFileName = getFinalCommitBodyFileName(
                                gitRepo.repoName,
                            )
                            writeStrToFile(
                                finalCommitEmailBodyFileName, finalCommitEmailBodyStr,
                            )
                            ...
                            commitAmendRtn = echoRunSysCmnd(
                                ...
                                " -F "
                                + os.path.join(
                                    baseTestDir, finalCommitEmailBodyFileName,
                                ),
                                workingDir=getGitRepoDir(
                                    inOptions.srcDir, gitRepo.repoDir,
                                ),
                                outFile=os.path.join(
                                    baseTestDir,
                                    getFinalCommitOutputFileName(gitRepo.repoName),
                                ),
                                timeCmnd=True,
                                throwExcept=False,
                            )
        ...
        if inOptions.doPushReadinessCheck:
            repoIdx = 0
            for gitRepo in tribitsGitRepos.gitRepoList():
                localCommitSummariesStr = getLocalCommitsSummariesStr(
                    inOptions, gitRepo,
                )
                    ...
                    if gitRepo.gitRepoStats.numCommitsInt() > 0:
                        if not debugSkipPush:
                            pushRtn = echoRunSysCmnd(
                                inOptions.git
                                + " push "
                                + pushToTrackingBranchArgs(gitRepo),
                                workingDir=getGitRepoDir(
                                    inOptions.srcDir, gitRepo.repoDir,
                                ),
                                outFile=os.path.join(
                                    baseTestDir, getPushOutputFileName(gitRepo.repoName),
                                ),
                                throwExcept=False,
                                timeCmnd=True,
                            )
        ...
        print("\n***")
        print(
            "*** 9) Create and send push (or readiness status) notification email  ...",
        )
        print("***\n")
        ...
        if inOptions.doPushReadinessCheck:
            ...
            if inOptions.sendEmailTo and abortGracefullyDueToNoUpdates:
                ...
                inOptions.sendEmailTo
                and allConfiguresAbortedDueToNoEnablesGracefullAbort
            ):
                print(
                ...
                if inOptions.sendEmailToOnPush and didPush:
                    emailAddresses += " " + getEmailAddressesSpaceString(
                        inOptions.sendEmailToOnPush,
                    )
                echoRunSysCmnd(
                    'mailx -s "'
                    + subjectLine
                    ...
                    + emailAddresses
                    + " < "
                    + summaryCommitEmailBodyFileName,
                )
