...
usageHelp = (
    r"""clone_extra_repos.py [options]
...
def getExtraReposDictListFromCmakefile(
    projectDir,
    extraReposFile,
    withCmake,
    extraReposType=g_extraReposTypeDefault,
    extraRepos="",
    tribitsDir=tribitsDir,
    verbose=True,
):
    cmnd = (
        '"'
    ...
    cmnd += " -P " + ciSupportDir + "/TribitsGetExtraReposForCheckinTest.cmake"
    rawOutput = getCmndOutput(cmnd, throwOnError=True, getStdErr=True)
    (headerOutput, extraReposPytonDictList) = getHeaderOutputAndExtraReposDictList(
        rawOutput,
    )
    if verbose:
        print("\n" + headerOutput)
...
def getGitoliteReposList(inOptions, trace=False, dumpGitoliteOutput=False):
    ...
    if trace and not dumpGitoliteOutput:
        verbose = True
    else:
        verbose = False
    return parseRawSshGitoliteRootInfoOutput(
        rawSshGitoliteRootInfoOutput, verbose=verbose,
    )
...
def filterOutMissingGitoliteRepos(extraRepoDictList_in, gitoliteRepos, verbose=False):
    gitoliteReposSet = set(gitoliteRepos)
    extraRepoDictList = []
    for extraReposDict in extraRepoDictList_in:
        extraRepoName = extraReposDict["NAME"]
        if extraRepoName not in gitoliteReposSet:
            if verbose:
                print(
                    "Excluding extra repo '"
                    + extraRepoName
...
def cloneExtraRepos(inOptions):
    ...
    if inOptions.gitoliteRoot:
        if verbLevelIsMinimal:
            ...
            print("***\n")
        gitoliteRepos = getGitoliteReposList(
            inOptions,
    ...
    if inOptions.createGitdistFile:
        ...
        createGitDistFile(
            extraRepoDictList, inOptions.createGitdistFile, verbose=verbLevelIsMinimal,
        )
