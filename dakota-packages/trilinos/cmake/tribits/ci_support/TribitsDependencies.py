...
def getDefaultDepsXmlInFile(rootDir, projectName):
    return os.path.join(
        getDefaultDepsXmlDirectory(rootDir), "%sPackageDependencies.xml" % projectName,
    )
...
def getDefaultCDashDepsXmlFile(rootDir):
    return os.path.join(
        getDefaultDepsXmlDirectory(rootDir), "CDashSubprojectDependencies.xml",
    )
...
class PackageDependencies:
    def __init__(
        ...
        packageDir_in,
        packageType_in,
        libRequiredDepPackages_in,
        libOptionalDepPackages_in,
        testRequiredDepPackages_in,
    ...
    def __str__(self):
        return (
            ...
            + str(self.packageDir)
            + ",\n"
            + "  libRequiredDepPackages="
            + str(self.libRequiredDepPackages)
            + ",\n"
            + "  libOptionalDepPackages="
            + str(self.libOptionalDepPackages)
            + ",\n"
            + "  testRequiredDepPackages="
            + str(self.testRequiredDepPackages)
            + ",\n"
            + "  testOptionalDepPackages="
            + str(self.testOptionalDepPackages)
            + " \n"
            + "  emailAddresses="
            + str(self.emailAddresses)
            + "\n"
            + "  parentPackage="
            + str(self.parentPackage)
            + "\n"
            + "}\n"
        )
...
class TribitsDependencies:
    ...
    def getPackagesNamesList(self, onlyTopLevelPackages=True):
        ...
        for packageDep in self.__packagesList:
            ...
            if packageDep.parentPackage == "" or not onlyTopLevelPackages:
                addPackage = True
            else:
                addPackage = False
            if addPackage:
    ...
    def updatePackageDepsCategory(
        ...
        for dep in depList:
            ...
            self.updatePackageDeps(
                libsOnly, packageRowID, dep_i, childDepStats, projectDepsTable,
            )

    def updatePackageDeps(
        self, libsOnly, packageRowID, packageID, depStats, projectDepsTable,
    ):
    ...
    def createRawTable(self, libsOnly):
        ...
        for packageDeps in self.__packagesList:
            ...
            self.updatePackageDeps(
                libsOnly, i, i, DepStats(True, True, False), projectDepsTable,
            )
    ...
    def createFullHtmlForTables(self):
        ...
        htmlText = (
            "<p><huge><b>"
            + self.getProjectName()
            + " Test/Example and Library Package Dependencies</b></huge></p>\n"
            + "\n"
            ...
            + "<p><b><huge>"
            + self.getProjectName()
            + " Libary-Only Package Dependencies</huge></b></p>\n"
            + "\n"
        ...
        for package_i in range(numPackages):
            ...
            if packageDeps.parentPackage == "":
                ...
                xmlText += (
                    "    <EmailAddresses>\n"
                     '      <Email address="'
                    + packageDeps.emailAddresses.regression
                    + '"/>\n'
                    + "    </EmailAddresses>\n"
                )
