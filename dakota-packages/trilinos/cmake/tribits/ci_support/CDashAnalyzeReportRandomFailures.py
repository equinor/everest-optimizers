...
class CDashAnalyzeReportRandomFailuresDriver:
    def __init__(self, versionInfoStrategy, extractBuildNameStrategy, usageHelp=""):
    ...
    def runDriver(self):
        ...
        dateRangeStart, dateRangeEnd = self.getDateRangeTuple(
            referenceDateDT, daysOfHistory,
        )
        ...
        cdashReportData.htmlEmailBodyTop += (
            "<h2>Random test failure scan results for "
            + cdashProjectName
            + " from "
            + dateRangeStr
            + "</h2>\n\n"
        ...
        for nonpassingTest in initialNonpassingTestsLOD:
            ...
            correctedBuildName = self.extractBuildNameStrategy.getCoreBuildName(
                nonpassingTest["buildName"],
            )

            testNameBuildName = (
                nonpassingTest["testname"] + "_" + nonpassingTest["buildName"]
            )
            ...
            testHistoryFilters = (
                "filtercount=3&showfilters=1&filtercombine=and"
                 "&field1=testname&compare1=63&value1="
                + nonpassingTest["testname"]
                + "&field2=groupname&compare2=63&value2="
                + groupNameNormUrl
                + "&field3=buildname&compare3=63&value3="
                + correctedBuildName
            )
            ...
            testHistoryCacheFile = (
                testHistoryCacheDir
                + "/"
                + CDQAR.getCompressedFileNameIfTooLong(
                    testNameBuildName + ".json", ext=".json",
                )
            )
            ...
            if printUrlMode == "all":
                print(
                    "\n  CDash test history browser URL for "
                    + nonpassingTest["testname"]
                    + " "
                    + correctedBuildName
                    + ":\n\n"
                ...
                print(
                    "\n  CDash test history query URL for "
                    + nonpassingTest["testname"]
                    + " "
                    + correctedBuildName
                    + ":\n\n"
            ...
            passingTestHistoryLOD = [
                test for test in testHistoryLOD if test.get("status") == "Passed"
            ]
            nonpassingTestHistoryLOD = [
                test for test in testHistoryLOD if test.get("status") == "Failed"
            ]
            nonpassingSha1Pairs = set()

            print(
                "\n  Num of passing tests in test history: "
                + str(len(passingTestHistoryLOD)),
            )
            print(
                "\n  Num of nonpassing tests in test history: "
                + str(len(nonpassingTestHistoryLOD)),
            )

            buildSummaryCacheDir = (
                testQueriesCacheDir + "/build_summary_cache/" + testNameBuildName
            )
            ...
            for test in passingTestHistoryLOD:
                ...
                passingSha1Pair = self.versionInfoStrategy.getTargetTopicSha1s(
                    buildConfigOutput,
                )

                if self.versionInfoStrategy.checkTargetTopicRandomFailure(
                    passingSha1Pair, nonpassingSha1Pairs,
                ):
                    print(
                        "\n  Found passing sha1 pair, "
                        + str(passingSha1Pair)
        ...
        print(
            "Total number of initial failing tests: "
            + str(len(initialNonpassingTestsLOD))
            + "\n",
        )
        ...
        if writeEmailToFile:
            ...
            htmlStr = CDQAR.getFullCDashHtmlReportPageStr(
                cdashReportData, pageTitle=subjectLine, pageStyle=defaultPageStyle,
            )
            # print(htmlStr)
    ...
    def getCmndLineArgs(self):
        ...
        parser.add_argument(
            ...
            default="",
            required=True,
            help="CDash project name (e.g. TriBITS)." + " [REQUIRED]",
        )
        parser.add_argument(
            "--initial-nonpassing-test-filters",
            default="",
            required=True,
        ...
        parser.add_argument(
            ...
            default="none",
            help="Mode of url printing." + " [default=none]",
        )
        parser.add_argument(
            "--write-email-to-file",
            default="",
            help="Name of file to write built email HTML to." + " [default='']",
        )
        parser.add_argument(
            "--send-email-to",
            default="",
        ...
        parser.add_argument(
            ...
            default="",
            help="Prefix string added to the email subject line." + " [default='']",
        )
