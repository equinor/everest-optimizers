...
"""
    ...
    def injectExtraCmndLineOptions(self, clp):
        clp.add_option(
            ...
            + self.getBaseDirName(),
            help="Command used to check out "
            + self.getProductName()
            + " and dependent source tarball(s).",
        )
        clp.add_option(
    ...
    def echoExtraCmndLineOptions(self, inOptions):
        ...
        if inOptions.extraConfigureOptions:
            cmndLine += (
                "  --extra-configure-options='"
                + inOptions.extraConfigureOptions
                + "' \\\n"
            )
    ...
    def doInstall(self):
        echoChDir(self.openmpiBuildBaseDir)
        echoRunSysCmnd(
            self.getEnvCmnd() + "make " + self.inOptions.makeOptions + " install",
        )

    def getFinalInstructions(self):
        return (
            """
To use the installed version of openmpi-"""
            + openmpiVersion
            + """ access the include
...
"""
    ...
    def getEnvCmnd(self):
        ...
        if self.inOptions.withLibDir:
            envCmnd += (
                "LD_LIBRARY_PATH=" + self.inOptions.withLibDir + ":$LD_LIBRARY_PATH "
            )
