    ...
    def runDriver(self):
        ...
        if productSupportedVersions:
            supportedVersionsTxt = "Versions supported include:\n   " + str(
                self.installObj.getProductSupportedVersions(),
            )
        else:
...
"""
            + productName
            + r""" in one shot.  """
            + supportedVersionsTxt
            + r"""

The version to install is set with --"""
            + productBaseName
            + r"""-version=<version>.
...
By default, if you just type:
   ...
   $ """
            + scriptName
            + r""" --install-dir=<install-dir> --parallel=<num-procs> --do-all

then the directory """
            + baseDirName
            + r""" will be created in the local working directory
and it will contain the source for """
            + productName
            + r""" and the build files. NOTE: This
...
which is set to /usr/local/bin by default.  For example, you might just type:
  ...
  $ """
            + scriptName
            + r""" --install-dir=$HOME/install --parallel=8 --do-all

and then it would install """
            + productName
            + r""" and the other executables in $HOME/install/bin.
...
that needs root privileges, do:
  ...
  $ """
            + scriptName
            + r""" --install-dir=$HOME/install --parallel=8 \
...
"""
        ...
        if supportedVersions:
            addOptionParserChoiceOption(
                ...
                supportedVersions,
                defaultVersionIdx,
                "Version to install for " + productName + ".",
                clp,
            )
        else:
            clp.add_option(
                ...
                help="Version to install for "
                + productName
                + " (list of versions open-ended).",
            )
        ...
        clp.add_option(
            ...
            type="string",
            default="",
            help="The install directory <install-dir> for "
            + productName
        ...
        clp.add_option(
            ...
            type="string",
            default="",
            help="The base install directory <install-dir> for "
            + productName
            + " (default = '').  In this case the subdir "
            + productName
...
def setStdDownloadCmndOption(installObj, clp, version):
    ...
    clp.add_option(
        ...
        help="Command used to download source for "
        + productName
        + "."
        + "  (Default ='"
        + defaultDownloadCmnd
...
def setStdGithubDownloadCmndOption(installObj, githubOrg, githubRepo, clp, version):
    ...
    clp.add_option(
        ...
        type="string",
        default=defaultDownloadCmnd,
        help="Command used to download source tarball for "
        + productName
        + "."
        + "  (Default ='"
        + defaultDownloadCmnd
