...
def generateFile(
    filePath, generateCmnd, outFile=None, workingDir="", runTwice=False, echoCmnds=False,
):
...
def addCmndLineOptions(clp):
    ...
    clp.add_option(
        "--generate-latex-options",
        dest="generateLatexOptions",
        type="string",
...
def generateDocutilsOuputFiles(options):
    ...
    if options.generateLatex:
        ...
        runSysCmnd(
            ...
            + " "
            + outputLatexFile,
            echoCmndForDebugging=options.echoCmnds,
        )
