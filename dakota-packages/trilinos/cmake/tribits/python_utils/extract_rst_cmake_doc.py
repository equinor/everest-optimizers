...
#
def extractRstDocBlocksFromText(
    rawText, rstBlockTypes, fileName, traceExtraction=False,
):
    rstDocBlocks = {}
...
def getRstDocBlock(rstDocBlocks, rstBlockName, fileNameAndLinePrefix, line):
    ...
    if rstDocBlock == "":
        raise Exception(
            ...
            + " '"
            + rstBlockName
            + "' does not exist in rstDocBlocks for line = '"
            + line
            + "'",
        )
    return rstDocBlock
...
#
def replaceWithRstDocBlocksInText(
    textToReplace,
    rstBlockTypes,
    rstDocBlocks,
    fileName,
    ...
    if not fileNamePathBaseDir == "":
        ...
        if not fileNamePathBaseDir.endswith("/"):
            fileNamePathBaseDirAndSepLen += 1
    else:
    ...
    for line in textToReplaceLines:
        ...
        for blockType in rstBlockTypes:
            ...
            if lineBlockBeginning == blockBeginning:
                ...
                if len(rstBlockSecChar) != 1:
                    raise Exception(
                        fileNameAndLinePrefix
                        + "Error, separation char must be on char in line = '"
                        + line
                        + "'",
                    )
                ...
                rstDocBlock = getRstDocBlock(
                    rstDocBlocks, rstBlockName, fileNameAndLinePrefix, line,
                )
                if rstDocBlock.get("type") != blockType:
                    raise Exception(
                        fileNameAndLinePrefix
                        + "Error, looked up block type"
                        + " '@"
                        + rstDocBlock.get("type")
                        + "' does not match block type for line = '"
                        + line
                        + "'",
                    )
        ...
        if not replacedRstBlock and not (
            line_i == numTextToReplaceLines and line == ""
        ):
            replacedText += line + "\n"
...
def extractRstDocBlocksFromFileList(fileList, rstBlockTypes, traceExtraction=False):
    ...
    for fileName in fileList:
        rawFileStr = open(fileName).read()
        rstDocBlocks.update(
            extractRstDocBlocksFromText(
                rawFileStr, rstBlockTypes, fileName, traceExtraction,
            ),
        )
    return rstDocBlocks
...
def replaceWithRstDocBlocksInTemplateFileList(
    rstFilesList,
    rstBlockTypes,
    rstDocBlocks,
    traceReplacements=False,
    ...
    for templateFileName, fileName in rstFilesList:
        templateFileStr = open(templateFileName).read()
        fileStr = replaceWithRstDocBlocksInText(
            templateFileStr,
            rstBlockTypes,
            rstDocBlocks,
            templateFileName,
            traceReplacements=traceReplacements,
...
if __name__ == "__main__":
    ...
    replaceWithRstDocBlocksInTemplateFileList(
        rstFilesList,
        rstBlockTypes,
        rstDocBlocks,
        traceReplacements=options.doTrace,
