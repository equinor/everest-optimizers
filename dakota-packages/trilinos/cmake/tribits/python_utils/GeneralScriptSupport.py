...
class SysCmndInterceptor:
    ...
    def nextInterceptedCmndStruct(self, cmnd):
        ...
        if len(self.__interceptedCmndStructList) == 0:
            raise Exception(
                "Error, cmnd='" + cmnd + "' is past the last expected command!",
            )
        ics = self.__interceptedCmndStructList[0]
        if not re.match(ics.cmndRegex, cmnd):
            raise Exception(
                "Error, cmnd='" + cmnd + "' did not match the"
    ...
    def readCommandsFromStr(self, cmndsStr):
        ...
        for line in lines:
            ...
            if tag == "FT":
                ...
                self.__interceptedCmndStructList.append(
                    InterceptedCmndStruct(
                        cmndRegex.strip(), int(cmndReturn), cmndOutput,
                    ),
                )
            else:
    ...
    def assertAllCommandsRun(self):
        if len(self.__interceptedCmndStructList) > 0:
            raise Exception(
                "Error, all of the commands have not been run starting with"
...
cmndInterceptsFile = os.environ.get("GENERAL_SCRIPT_SUPPORT_CMND_INTERCEPTS_FILE", "")
if cmndInterceptsFile:
    cmndInterceptsFileStr = open(cmndInterceptsFile).read()
    print(
...
# (i.e. 'utf-8')
#
def runSysCmndInterface(
    cmnd, outFile=None, rtnOutput=False, extraEnv=None, workingDir="", getStdErr=False,
):
    ...
    if g_sysCmndInterceptor.doProcessInterceptedCmnd(cmnd):
        ...
        if rtnOutput:
            if cmndOutput == None:
                raise Exception(
                    "Error, the command '" + cmnd + "' gave None output when"
    ...
    try:
        if rtnOutput:
            if getStdErr:
                ...
                child = subprocess.Popen(
                    cmnd, shell=True, stdout=subprocess.PIPE, env=fullEnv,
                )
...
def runSysCmnd(
    ...
    try:
        ...
        if echoCmndForDebugging:
            print("Running: " + cmnd)
        rtnCode = runSysCmndInterface(
            cmnd, outFile=outFile, extraEnv=extraEnv, workingDir=workingDir,
        )
    except OSError:
        rtnCode = 1  # Just some error code != 0 please!
    if rtnCode != 0 and throwExcept:
        raise RuntimeError(
            "Error, the command '%s' failed with error code %d" % (cmnd, rtnCode),
        )
    return rtnCode
...
def getCmndOutput(
    ...
    (data, errCode) = runSysCmndInterface(
        cmnd, rtnOutput=True, workingDir=workingDir, getStdErr=getStdErr,
    )
    if errCode != 0:
        if throwOnError:
            raise RuntimeError(
                "%s failed w/ exit code %d:\n\n%s" % (cmnd, errCode, data),
            )
    dataToReturn = data
...
def getFileNamesWithFileTag(baseDir, fileTag):
    ...
    return getCmndOutput(
        "cd %s && ls *%s*" % (baseDir, fileTag), throwOnError=False,
    ).split()
...
def getDirSizeInGb(dir):
    ...
    return float(sizeIn1024Kb) / 1e6  # Size in Gb!
...
def getRelativePathFrom1to2(absPath1, absPath2):
    ...
    baseDirDepth = 0
    for dirIdx in range(maxBaseDirDepth):
...
def copyFileAndReplaceTokens(scriptsDir, inputFile, tokenReplacementList, outputFile):
...
def createIndexHtmlBrowserList(baseDir, fileDirList=None):
    ...
    for fd in fileDirList:
        ...
        if not os.path.isfile(absFd):
            htmlList = htmlList + '<li>dir: <a href="' + fd + '">' + fd + "</a></li>\n"
    ...
    for fd in fileDirList:
        ...
        if os.path.isfile(absFd):
            if fd != "index.html":
                htmlList = (
                    htmlList + '<li>file: <a href="' + fd + '">' + fd + "</a></li>\n"
                )
    # Write the footer
