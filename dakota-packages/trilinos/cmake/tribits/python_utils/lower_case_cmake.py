...
commandCallRegex = re.compile(r"[a-zA-Z_][a-zA-Z0-9_]*\(")
commandCallWithSpacesRegex = re.compile(r"[a-zA-Z_][a-zA-Z0-9_]*[\s]*\(")
macroDefRegex = re.compile(
    r"[^a-zA-Z_][mM][aA][cC][rR][oO]\([\s]*[a-zA-Z_][a-zA-Z0-9_]*",
)
functionDefRegex = re.compile(
    r"[^a-zA-Z_][fF][uU][nN][cC][tT][iI][oO][nN]\([\s]*[a-zA-Z_][a-zA-Z0-9_]*",
)
...
for specialCommandName in specialCommandNamesList:
    specialCommandRegexList.append(re.compile(specialCommandName + r"[\s]*\("))
...
def makeCmndsLowerCaseInCMakeStr(cmakeCodeStrIn):
    ...
    cmakeCodeStrOut = makeRegexMatchesLowerCaseInCMakeStr(
        cmakeCodeStrOut, commandCallWithSpacesRegex,
    )
    cmakeCodeStrOut = makeRegexMatchesLowerCaseInCMakeStr(
        cmakeCodeStrOut, macroDefRegex,
    )
    cmakeCodeStrOut = makeRegexMatchesLowerCaseInCMakeStr(
        cmakeCodeStrOut, functionDefRegex,
    )
...
def makeRegexMatchesLowerCaseInCMakeStr(cmakeCodeStrIn, regex):
    ...
    while cmakeCodeStrStartSearchIdx < cmakeCodeStrInLen:
        ...
        if m:
            ...
            commandCallMatchStr = cmakeCodeStrIn[
                cmakeCodeStrMatchStartIdx:cmakeCodeStrMatchEndIdx
            ]
            assert m.group() == commandCallMatchStr
            # Lower case the matching command call and update the output string
            # since last match
            cmakeCodeStrOut += cmakeCodeStrIn[
                cmakeCodeStrStartSearchIdx:cmakeCodeStrMatchStartIdx
            ]
            ...
            cmakeCodeStrOut += cmakeCodeStrIn[cmakeCodeStrStartSearchIdx:]
            break
...
def isConditionalCMakeGroup(cmakeMatchGroup):
    if (
        andLogicalRegex.match(cmakeMatchGroup)
...
def getCmndLineOptions():
    ...
    clp.add_argument("file", help="The file to lower-case the CMake commands within.")
...
def lowerCaseCMakeCodeInFile(inOptions):
    with open(inOptions.file) as fileHandle:
