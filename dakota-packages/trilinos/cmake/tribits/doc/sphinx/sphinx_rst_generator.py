...
def change_paths_and_get_includes(
    source_file: str,
    src_file_path: str,
    start_path: str,
    rst_dir: str,
    tribits_base_dir: str,
    copy_file: bool = True,
) -> tuple:
        ...
        for line in source_file_str.split("\n"):
            ...
            if "include::" in splitted_line:
                ...
                if len(splitted_line) > path_index:
                    new_line = []
                    spaces_indented = line.find(
                        ".",
                    )  # Below 'join()' statement adds a space!
                    if spaces_indented > 0:
                        new_line.append(" " * (spaces_indented - 1))
                    new_line.extend(splitted_line[:path_index])
                    abs_path = os.path.abspath(
                        os.path.join(src_file_path, splitted_line[path_index]),
                    )
                    ...
                    rel_path_from_sphinx_dir = os.path.relpath(
                        path=new_path, start=start_path,
                    )
...
class SphinxRstGenerator:
        ...
        self.paths = {
            "maintainers_guide": {
                ...
                "final_path": os.path.join(
                    doc_path, "sphinx", "maintainers_guide", "index.rst",
                ),
            ...
            "users_guide": {
                "src": os.path.join(
                    doc_path, "guides", "users_guide", "TribitsUsersGuide.rst",
                ),
                "src_path": os.path.join(doc_path, "guides", "users_guide"),
                "final_path": os.path.join(
                    doc_path, "sphinx", "users_guide", "index.rst",
                ),
            ...
            "build_ref": {
                ...
                "final_path": os.path.join(
                    doc_path, "sphinx", "build_ref", "index.rst",
                ),
    ...
    def combine_documentation(
        self,
        docs_dir: str,
        change_url_to_landing_page: bool = True,
        change_title_of_docs_main_page: bool = True,
        title: str = "",
    ) -> None:
        ...
        if change_title_of_docs_main_page:
            self.change_title_of_docs_main_page(
                docs_static_dir=static_dir, new_title=title,
            )
    ...
    def change_url_to_landing_page(docs_static_dir: str) -> None:
        ...
        with open(index_html) as index_read:
            index_str = index_read.read()
        repl_url = index_str.replace(
            '<a href="#" class="icon icon-home"> TriBITS',
    ...
    def change_title_of_docs_main_page(docs_static_dir: str, new_title: str) -> None:
        ...
        with open(index_html) as index_read:
            index_str = index_read.read()
        repl_url = re.sub(
            "(<title>1 Introduction &mdash;).+(documentation</title>)",
    ...
    def generate_rst(
        ...
        file_content, includes = change_paths_and_get_includes(
            source_file=source_file,
            src_file_path=src_path,
            start_path=start_path,
            rst_dir=self.rst_dir,
    ...
    def remove_title_numbering(self) -> None:
        ...
        for doc_name, sources in self.paths.items():
            ...
            with open(sources.get("final_path")) as src_file:
    ...
    def main(self):
        ...
        child_rst = set()
        for doc_name, sources in self.paths.items():
            includes = self.generate_rst(
                source_file=sources.get("src"),
                src_path=sources.get("src_path"),
                final_path=sources.get("final_path"),
                start_path=sources.get("sphinx_path"),
            )
        ...
        for child in child_rst_lst:
            ...
            includes_grand = self.generate_rst(
                source_file=child,
                src_path=src_path,
        ...
        grand_child_rst_lst = [
            gc_rst
            for gc_rst in grand_child_rst
            if gc_rst not in self.already_modified_files
        ]
        ...
        for grand_child in grand_child_rst_lst:
            ...
            includes_grand_grand = self.generate_rst(
                source_file=grand_child,
                src_path=src_path,
                final_path=final_path,
                start_path=sphinx_rel_path,
            )
