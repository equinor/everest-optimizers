...
def mock_subprocess_check_call(*args, **kwargs):
    ...
    for k, v in kwargs.items():  # pragma: no cover
        params.append(f"{k}={v}")  # pragma: no cover
    output = "--- subprocess.check_call({})".format(
        ", ".join(params),
    )  # pragma: no cover
...
def mock_subprocess_raise_CalledProcessError(*args, **kwargs):
    ...
    print(f"    : Command is `{cmd}`")  # pragma: no cover
...
def mock_modulehelper_module_ok(*args, **kwargs):
    ...
    print(f"MOCK> module({cmd})    QAPLA'!")  # pragma: no cover
...
def mock_modulehelper_module_fail(*args, **kwargs):
    ...
    """
    cmd = ", ".join([f"'{x}'" for x in args])
    print(f"MOCK> module({cmd})  # FAILS")  # pragma: no cover
...
def mock_se_apply_pass(*args, **kwargs):
    ...
    print(f"MOCK> SetEnvironment.apply({params})")  # pragma: no cover
    return 0  # pragma: no cover
...
class TrilinosPRConfigurationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        print()
        print(f"--- LoadEnv Config file found: {self._env_config_file}")
        print(f"--- GenConfig Config file found: {self._gen_config_file}")
        print()
    ...
    def test_TrilinosPRConfigurationBase(self):
        ...
        print(f"pr_config.max_cores_allowed    = {pr_config.max_cores_allowed}")
        print(
            f"pr_config.arg_req_mem_per_core = {pr_config.arg_req_mem_per_core}",
        )
        print(
            f"pr_config.max_test_parallelism = {pr_config.max_test_parallelism}",
        )
        print(f"pr_config.concurrency_build    = {pr_config.concurrency_build}")
        print(f"pr_config.concurrency_test     = {pr_config.concurrency_test}")
    ...
    def test_TrilinosPRConfigurationCDashTrack(self):
        ...
        print(f"--- cdash_track = {cdash_track}")
    ...
    def test_TrilinosPRConfigurationBaseDashboardModelPRTrack(self):
        ...
        print(f"--- dashboard_model = {dashboard_model}")
            ...
            with self.assertRaises(subprocess.CalledProcessError) as m_err:
                with patch(
                    ...
                    with patch(
                        ...
                        side_effect=mock_subprocess_raise_CalledProcessError,
                    ) as m_output:
                        pr_config.create_package_enables_file(dryrun=False)
    ...
    def test_TrilinosPRConfigurationBaseProperty_concurrency_test_err(self):
        ...
        print(f"concurrency_test = {concurrency_test}")
    ...
    def test_TrilinosPRConfigurationBaseProperty_get_property_from_config_PASS(self):
        ...
        package_enables = pr_config.get_property_from_config(
            "ENABLE_MAP", args.pullrequest_build_name,
        )

        print(f"--- package_enables = {package_enables}")
    ...
    def test_TrilinosPRConfigurationBaseProperty_get_property_from_config_FAIL(self):
        ...
        with patch("sys.stdout", new=StringIO()) as fake_out:
            package_enables = pr_config.get_property_from_config(
                "NONEXISTENTSECTION", "N/A",
            )
        print(fake_out.getvalue())
        print(f"--- package_enables = {package_enables}")
        print(f"--- expected_value  = {None}")
        ...
        with patch("sys.stdout", new=StringIO()) as fake_out:
            package_enables = pr_config.get_property_from_config(
                "NONEXISTENTSECTION", "N/A", default=default_value,
            )
        print(fake_out.getvalue())
        print(f"--- package_enables = {package_enables}")
        print(f"--- expected_value  = {default_value}")
        self.assertEqual(package_enables, default_value)
        self.assertIn("WARNING", fake_out.getvalue())

    def test_TrilinosPRConfigurationBaseProperty_get_multi_property_from_config_PASS(
        self,
    ):
        print()
        args = self.dummy_args()
        ...
        print("-----[ TEST 1 ]-----------------------")
        package_enables = pr_config.get_multi_property_from_config(
            "ENABLE_MAP", args.pullrequest_build_name,
        )
        print(f"--- package_enables = {package_enables}")
        ...
        print("-----[ TEST 2 ]-----------------------")
        package_enables = pr_config.get_multi_property_from_config(
            "ENABLE_MAP", args.pullrequest_build_name, delimeter=" ",
        )
        print(f"--- package_enables = {package_enables}")
        print("--- expected_value  = Teuchos Tpetra")
        self.assertEqual(package_enables, "Teuchos Tpetra")

    def test_TrilinosPRConfigurationBaseProperty_get_multi_property_from_config_FAIL(
        self,
    ):
        ...
        print("-----[ TEST 1 ]-----------------------")
        package_enables = pr_config.get_multi_property_from_config(
            "NONEXISTENTSECTION", args.pullrequest_build_name,
        )
        print(f"--- package_enables = {package_enables}")
        self.assertEqual(package_enables, None)
        ...
        print("-----[ TEST 2 ]-----------------------")
        package_enables = pr_config.get_multi_property_from_config(
            "NONEXISTENTSECTION", args.pullrequest_build_name, default="OOPS!",
        )
        print(f"--- package_enables = {package_enables}")
    ...
    def test_TrilinosPRConfigurationBaseProperty_subprojects_file(self):
        ...
        print(f"--- filename_subprojects = {filename_subprojects}")
    ...
    def test_TrilinosPRConfigurationBaseProperty_package_enables_file(self):
        ...
        print(f"--- package_enables_file = {package_enables_file}")
    ...
    def test_TrilinosPRConfigurationBase_prepare_test(self):
        ...
        with patch(
            "trilinosprhelpers.setenvironment.ModuleHelper.module",
    ...
    def test_TrilinosPRConfigurationBase_prepare_test_skip_create_package_enables_file(
        self,
    ):
    ...
    def test_TrilinosPRConfigurationBase_prepare_test_FAIL(self):
        ...
        with patch(
            ...
            with patch(
                "trilinosprhelpers.setenvironment.SetEnvironment.apply",
