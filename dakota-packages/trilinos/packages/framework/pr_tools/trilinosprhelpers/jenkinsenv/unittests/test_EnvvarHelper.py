...
from trilinosprhelpers import jenkinsenv
...
class EnvvarHelperTest(TestCase):
    ...
    def test_envvar_notfound_with_error(self):
        with self.assertRaises(KeyError):
            self._envvar_helper.get_envvar_str(
                self._bogus_envvar, error_if_missing=True,
            )

    def test_get_or_create_if_missing(self):
        value = self._envvar_helper.get_or_create_if_missing(
            "__TEST_ENVVAR__", default_value="__NEW_VALUE__",
        )
        self.assertEqual(value, "__TEST_VALUE__")

        value = self._envvar_helper.get_or_create_if_missing(
            "__TEST_MISSING_ENVVAR__", default_value="__NEW_VALUE__",
        )
