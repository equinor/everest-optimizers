...
class GitUtility:
    ...
    def check_minimum_version(self, req_major, req_minor=0):
        ...
        str_err = f"Required Git version must be greater or equal to {req_major}.{req_minor}\n"
        str_err += f"but we detected version {det_major}.{det_minor}"

        if det_major < req_major or (det_major == req_major and det_minor < req_minor):
    ...
    def pretty_print(self):
        ...
        """
        print()
        print(f"Git Version Detected: {self.version_str}")
        print()
