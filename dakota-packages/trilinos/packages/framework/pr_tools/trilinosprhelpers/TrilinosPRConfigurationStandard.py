...
class TrilinosPRConfigurationStandard(TrilinosPRConfigurationBase):
    ...
    def execute_test(self):
        ...
        if self.arg_skip_create_packageenables:
            print(
                "Optional --skip_create_packageenables found. "
        ...
        if "BUILD_NUMBER" in os.environ:
            print(
                "Running under Jenkins, keeping output less verbose to avoid space issues",
            )
            verbosity_flag = "-V"
        ...
        cmd = [
            ...
            "-DUSE_EXPLICIT_TRILINOS_CACHEFILE:BOOL="
            + ("ON" if self.arg_use_explicit_cachefile else "OFF"),
