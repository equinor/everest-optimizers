...
def echoJenkinsVars(workspace):
    """ """
    print_wrapper(80 * "-")
    print_wrapper("Jenkins Environment Variables:")
    print_wrapper(f"WORKSPACE: {workspace}")
    print_wrapper("Environment:")
    print_wrapper(f"-- pwd = {os.getcwd()}")
    print_wrapper("Environment Variables:")
...
def parseArgs():
    ...
    parser.add_argument("sourceRepo", help="Repo with the new changes", action="store")
    parser.add_argument("targetRepo", help="Repo to merge into", action="store")
    parser.add_argument("targetBranch", help="Branch to merge to", action="store")
    parser.add_argument(
        "sourceSHA",
    ...
    parser.add_argument(
        "workspaceDir", help="The local workspace directory jenkins set up",
    )
...
def merge_branch(source_url, target_branch, sourceSHA):
    ...
    sha_exists_as_branch_on_remote = bool(
        subprocess.check_output(
            "git rev-parse --verify --quiet source_remote/" + sourceSHA + " || true",
            shell=True,
        ),
    )
    ...
    if sha_exists_as_branch_on_remote:
        ...
        check_call_wrapper(
            ["git", "merge", "--no-ff", "--no-edit", "source_remote/" + sourceSHA],
        )
    else:
...
def run():
    ...
    if return_value:
        ...
        try:
            merge_branch(
                arguments.sourceRepo, arguments.targetBranch, arguments.sourceSHA,
            )
        except SystemExit:
            return_value = False
            ...
            print_wrapper(
                f"Received subprocess.CalledProcessError - returned {cpe.returncode}",
            )
...
if __name__ == "__main__":  # pragma nocover
    returnValue = run()
    if returnValue:
        print_wrapper(" Finished Normally")
        exit(0)
    else:
        print_wrapper(" Finished with error(s)")
        exit(1)
