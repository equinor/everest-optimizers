...
def parse_args():
    ...
    desc_package_enables = (
        "The packageEnables.cmake is usually generated by TriBiTS infrastructure "
         "based on which packages contain the changes between the source and target "
         "branches."
    )

    optional.add_argument(
        "--filename-packageenables",
        dest="filename_packageenables",
        action="store",
        default=default_filename_packageenables,
        help=f"{desc_package_enables} Default={default_filename_packageenables}",
    )
    ...
    desc_subprojects_file = (
        "The subprojects_file is used by the testing infrastructure. This parameter "
    ...
    optional.add_argument(
        ...
        help=f"{desc_subprojects_file}. Default={default_filename_subprojects}",
    )
    ...
    print("+" + "=" * 78 + "+")
    print(
        "| - [R] source-repo-url             : {source_repo_url}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [R] target_repo_url             : {target_repo_url}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [R] target_branch_name          : {target_branch_name}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [R] pullrequest-build-name      : {pullrequest_build_name}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [R] genconfig-build-name        : {genconfig_build_name}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [R] pullrequest-number          : {pullrequest_number}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [R] jenkins-job-number          : {jenkins_job_number}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [R] source-dir                  : {source_dir}".format(**vars(arguments)),
    )
    print("| - [R] build-dir                   : {build_dir}".format(**vars(arguments)))
    print(
        "| - [R] ctest-driver                : {ctest_driver}".format(**vars(arguments)),
    )
    print(
        "| - [R] ctest-drop-site             : {ctest_drop_site}".format(
            **vars(arguments),
        ),
    )
    print("|")
    ...
    print(
        "| - [O] enable-ccache               : {ccache_enable}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [O] filename-packageenables     : {filename_packageenables}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [O] max-cores-allowed           : {max_cores_allowed}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [O] num-concurrent-tests        : {num_concurrent_tests}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [O] pullrequest-cdash-track     : {pullrequest_cdash_track}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [O] pullrequest-env-config-file : {pullrequest_env_config_file}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [O] pullrequest-gen-config-file : {pullrequest_gen_config_file}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [O] req-mem-per-core            : {req_mem_per_core}".format(
            **vars(arguments),
        ),
    )
    print("| - [O] test-mode                   : {test_mode}".format(**vars(arguments)))
    print(
        "| - [O] workspace-dir               : {workspace_dir}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [O] skip-run-tests              : {skip_run_tests}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [O] extra_configure_args        : {extra_configure_args}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [O] dashboard_build_name        : {dashboard_build_name}".format(
            **vars(arguments),
        ),
    )
    print(
        "| - [O] use_explicit_cachefile       : {use_explicit_cachefile}".format(
            **vars(arguments),
        ),
    )
