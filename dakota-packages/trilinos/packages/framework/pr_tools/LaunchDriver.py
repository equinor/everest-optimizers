...
try:  # pragma: no cover
    ...
    try:  # Perhaps this file is being imported from another directory
        ...
        ImportError
    ):  # Perhaps LoadEnv was snapshotted and these packages lie up one dir.
...
def main(argv):
    ...
    parser = argparse.ArgumentParser(
        description="Launch a trilinos driver script on this system.",
    )
    parser.add_argument(
        "--build-name", required=True, help="The name of the build being launched",
    )
    parser.add_argument(
        "--driver",
        required=False,
    ...
    print(
        'LaunchDriver> INFO: TRILINOS_DIR="' + os.environ["TRILINOS_DIR"] + '"',
        flush=True,
    )
    ...
    if args.build_name.startswith("ats2_cuda"):
        args.driver = (
            "./Trilinos/packages/framework/pr_tools/PullRequestLinuxCudaVortexDriver.sh"
        )
    ...
    cmd_output = subprocess.run(cmd, check=False, shell=True)
