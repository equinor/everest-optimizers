...
class Test_parsing(unittest.TestCase):
    ...
    def test_parseArgs(self):
        ...
        test_namespace.sourceRepo = "/dev/null/source/Trilinos.git"
        test_namespace.targetRepo = "/dev/null/target/Trilinos.git"
        test_namespace.targetBranch = "fake_develop"
        test_namespace.sourceSHA = "0123456789abcdef"
        test_namespace.workspaceDir = "/dev/null/workspace"
        ...
        with mock.patch.object(
            ...
            [
                "programName",
                os.path.join(os.path.sep, "dev", "null", "source", "Trilinos.git"),
                os.path.join(os.path.sep, "dev", "null", "target", "Trilinos.git"),
                "fake_develop",
                "0123456789abcdef",
...
"""
        with mock.patch.object(sys, "argv", ["programName"]), mock.patch(
...
class Test_mergeBranch(unittest.TestCase):
    ...
    def test_mergeBranch_without_source_remote(self):
        ...
        ) as m_check_out, mock.patch("subprocess.check_call") as m_check_call:
            PRMerge.merge_branch(
                os.path.join(os.path.sep, "dev", "null", "source", "Trilinos.git"),
                "fake_develop",
                "df324ae",
            )
    ...
    def test_mergeBranch_with_source_remote(self, m_check_call):
        ...
        with mock.patch(
            "subprocess.check_output", side_effect=side_effect_list,
        ) as m_check_out:
            with mock.patch("sys.stdout", new_callable=StringIO) as m_stdout:
        ...
        expected_calls.append(
            mock.call(
                "git rev-parse --verify --quiet source_remote/df324ae || true",
                shell=True,
            ),
        )
    ...
    def test_mergeBranch_ref_is_remote_branch(self, m_check_call):
        ...
        m_check_call.assert_has_calls(
            [
                ...
                mock.call(
                    ["git", "merge", "--no-ff", "--no-edit", "source_remote/some_ref"],
                ),
            ],
        )
        self.assertIn("git remote exists, removing it", m_stdout.getvalue())
    ...
    @patch(
        "subprocess.check_output",
        side_effect=["origin /dev/null/target/Trilinos", "df324ae"],
    )
    def test_mergeBranch_fails_on_source_fetch(self, m_check_out):
        ...
        with self.assertRaises(SystemExit):
            with mock.patch(
                "subprocess.check_call", side_effect=side_effect_list,
            ) as m_check_call:
                PRMerge.merge_branch(tmp_path, "fake_develop", "df324ae")
...
class Test_run(unittest.TestCase):
    ...
    @patch("os.path.join")
    def test_run(
        self, m_join, m_mergeBranch, m_echoJenkins, m_writeHeader, m_chdir, m_parser,
    ):
        ...
        m_mergeBranch.assert_called_once_with(
            m_parser().sourceRepo, m_parser().targetBranch, m_parser().sourceSHA,
        )
    ...
    def test_run_fails_on_bad_parse(self):
        with mock.patch(
            "PullRequestLinuxDriverMerge.parseArgs", side_effect=SystemExit(2),
        ):
    ...
    def test_run_fails_on_bad_fetch(self, m_parseArgs, m_chdir, m_join):
        ...
        with mock.patch(
            ...
            with mock.patch(
                "subprocess.check_call", side_effect=side_effect_check_call,
            ):
    ...
    def test_run_fails_on_bad_remote_add(
        self, m_stdout, m_parseArgs, m_chdir, m_path_join,
    ):
