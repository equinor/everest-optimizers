...
class Test_parse_args(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.m_argv = mock.patch.object(
            sys,
            "argv",
            [
                "programName",
                "--source-repo-url",
                os.path.join(os.path.sep, "dev", "null", "source_repo"),
                "--target-repo-url",
                os.path.join(os.path.sep, "dev", "null", "target_repo"),
                "--target-branch-name",
                "real_trash",
        ...
        self.default_options = Namespace(
            source_repo_url="/dev/null/source_repo",
        ...
        self.m_cwd = mock.patch(
            "PullRequestLinuxDriverTest.os.getcwd",
            return_value=os.path.join(os.path.sep, "dev", "null", "Trilinos_clone"),
        )
        self.m_cwd.start()
    ...
    def test_parse_args_uses_workspace_environ(self):
        """No inputs"""
        tmp_workspace_dir = os.path.join(
            os.path.sep, "dev", "null", "Trilinos_workspace",
        )
        ...
        l_stdout = l_stdout.replace(
            "workspace-dir               : /dev/null/Trilinos_clone",
            "workspace-dir               : /dev/null/Trilinos_workspace",
        )

        with self.m_argv, mock.patch.dict(
            os.environ, {"WORKSPACE": tmp_workspace_dir}, clear=True,
...
class Test_main(unittest.TestCase):
    ...
    def setUp(self):
        self.prstd_mock = mock.patch(
            "PullRequestLinuxDriverTest.trilinosprhelpers."
            "TrilinosPRConfigurationStandard",
        )
        self.prinstall_mock = mock.patch(
            "PullRequestLinuxDriverTest.trilinosprhelpers."
    ...
    def test_calls_standard(self):
        ...
        """
        m_args = Namespace()
        m_args.dry_run = False
        m_args.test_mode = "standard"
        ...
        self.assertIn(
            "P U L L R E Q U E S T   D R I V E R   S T A R T", m_io.getvalue(),
        )
    ...
    def test_raises_on_unkown(self):
        ...
        m_args = Namespace()
        m_args.dry_run = False
        m_args.test_mode = "foobar"

        with self.prinstall_mock as m_pr_helper, self.IOredirect as m_io, self.assertRaisesRegex(
            KeyError, "ERROR: Unknown test mode, foobar, was provided.",
        ):
