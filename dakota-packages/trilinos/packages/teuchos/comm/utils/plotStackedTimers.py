...
try:
    from sunburst import Path, stringvalues_to_pv
    from sunburst import SunburstPlot as sbplot
except ImportError:
    print(
        "This scripts needs the python package from https://github.com/klieret/sunburst",
    )
    raise
...
parser.add_argument(
    "--root",
    help="Use a different node as root. Use format 'Level 0 timer name/Level 1 timer name'",
    dest="root",
    default="",
)
parser.add_argument(
    "-N",
    "--useTimerNumbers",
    help="Use timer numbers in plot instead of labels.",
    dest="useTimerNumbers",
    action="store_true",
    default=False,
)
...
timerRegExp = re.compile(
    r"([\s|]*)(.*):\s([0-9]*\.[0-9]*)\s(-\s([0-9]*\.[0-9]*)%\s)*\[[0-9]+\]",
)
...
if options.useTimerNumbers:
    ...
    s = "\n".join(
        [
            str(translate[key]) + ": " + key
            for key in sorted(translate, key=lambda key: int(translate[key]))
        ],
    )
    print(s)
