...
def plotMem(
    ...
    print(f"maximum heap memory usage: {memHeap.max()}")
    times = times[idx]
    memHeap = memHeap[idx]
    ...
    if displayTimers:
        ...
        for i, pair in enumerate(sorted(snapshotPairs, key=lambda x: x[0]["time"])):
            if (
                filter.search(pair[0]["label"])
                and abs(pair[0]["mem_heap"] - pair[1]["mem_heap"]) > minMemDiff
            ):
                t = [pair[0]["time"], pair[1]["time"]]
                if (
                    abs(t[0] - told[0]) < minTimeDiff
                    and abs(t[1] - told[1]) < minTimeDiff
                ):
                    print(
                        'Timers "{}" and "{}" seems to coincide'.format(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""Plot memory profile from massif.
...
are also included.""",
    ...
    parser.add_argument(
        "massifFile", nargs=1, help='massif log file, something like "massif.out.PID"',
    )
    parser.add_argument(
        "--minMemDiff",
    ...
    parser.add_argument(
        "--filter", help="regexp to filter for timers to include", type=str, default="",
    )
    parser.add_argument(
        "--memUnit", help="memory unit (KB, MB)", type=str, default="MB",
    )
