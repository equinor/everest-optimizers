...
def get_rol_headers(rol_path: Path, token: str) -> [Path]:
    result = sp.Popen(
        ["grep", "-rl", token, "--include=*.hpp", rol_path], stdout=sp.PIPE,
    )
...
def strip_cpp_comments(cpp_source: str) -> str:
    ...
    while i < len(cpp_source):
        ...
        if cpp_source[i] == '"' and not (
            in_single_line_comment or in_multi_line_comment
        ):
        ...
        # Check for single-line comment start
            ...
            i + 1 < len(cpp_source)
            and cpp_source[i : i + 2] == "//"
            and not (in_string or in_multi_line_comment)
        ):
        ...
        # Check for multi-line comment start
            ...
            i + 1 < len(cpp_source)
            and cpp_source[i : i + 2] == "/*"
            and not (in_string or in_single_line_comment)
        ):
