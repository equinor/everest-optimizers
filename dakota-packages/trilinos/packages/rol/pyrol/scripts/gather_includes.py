...
def copy_and_angular_includes(filenames, filenames_witout_dir, to_dir):
    ...
    for i in range(len(filenames)):
        ...
        try:
            with open(filename) as from_f:
                lines = from_f.readlines()
        except UnicodeDecodeError:
            with open(filename, encoding="iso-8859-1") as from_f:
                lines = from_f.readlines()
...
if __name__ == "__main__":
    ...
    with open(all_header_list_with_dir) as fh:
        all_include_filenames_with_dir = fh.read().splitlines()

    with open(all_header_list_without_dir) as fh:
        all_include_filenames_without_dir = fh.read().splitlines()

    copy_and_angular_includes(
    ...
    make_all_includes_from_filenames(
        binder_include_name, [CMAKE_CURRENT_SOURCE_DIR + "/src/Binder_Input.hpp"],
    )
