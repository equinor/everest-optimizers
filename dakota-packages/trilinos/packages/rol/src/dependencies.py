...
def includes(pathfile):
    ...
    if os.path.exists(pathfile):
        ...
        with open(pathfile) as f:
            lines = [
                line for line in f if "#include" in line and "ROL_" in line
            ]
...
def get_headers(cwd, filename, opened_headers=list()):
    ...
    new_headers = [
        h for h in includes(get_pathfile(cwd, filename)) if h not in opened_headers
    ]
...
if __name__ == "__main__":
    ...
    msg = f"Checking dependencies for file {filename}\n\n"
    print(msg)
