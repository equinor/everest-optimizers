...
@pytest.mark.xfail(
    reason="Something is wrong in the implementation. Produced output does not match expected output."
)
...
def test_complex_problem_parameterized_start(x0: NDArray[np.float64]):
    ...
    def objective_grad(x: NDArray[np.float64]) -> NDArray[np.float64]:
        return np.array(
            [2 * (x[0] - 1), 2 * (x[1] - 2.5), 2 * (x[2] - 2), 2 * (x[3] - 0.5)]
        )

    # Constraints: x[0] - 2*x[1] = 0, x[2] + x[3] = 1
    constraints = LinearConstraint(
        np.array([[1, -2, 0, 0], [0, 0, 1, 1]]),
        lb=np.array([0, 1]),
        ub=np.array([0, 1]),
    )
