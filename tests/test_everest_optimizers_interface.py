...
class TestEverestOptimizersInterface:
    ...
    def test_minimize_function_signature(self):
        ...
        expected_params = [
            'fun', 'x0', 'args', 'method', 'jac', 'hess', 'hessp', 
    ...
    def test_unsupported_features_error(self):
        ...
        # Test bounds
        with pytest.raises(NotImplementedError, match="OptQNewton does not support bounds"):
            minimize(dummy_func, [1.0], method='OptQNewton', bounds=[(0, 1)])
        
        # Test constraints
        with pytest.raises(NotImplementedError, match="OptQNewton does not support constraints"):
            minimize(dummy_func, [1.0], method='OptQNewton', constraints={'type': 'eq', 'fun': lambda x: x[0]})
        
        # Test callback
        with pytest.raises(NotImplementedError, match="Callback function not implemented"):
