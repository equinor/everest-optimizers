...
void linear_eq_test1_obj(
    int mode, int n, const SerialDenseVector<int, double>& x, double& fx,
...
TestResult run_linear_eq_test1() {
  ...
  try {
    ...
    result.constraint_violation =
        check_constraint_violation(result.final_point, create_linear_eq_test1_constraints(2));
...
TestResult run_linear_eq_test2() {
  ...
  try {
    ...
    result.final_objective = nlp.getF();
    result.constraint_violation =
        check_constraint_violation(result.final_point, create_linear_eq_test2_constraints(3));
    result.iterations = optimizer.getIter();
...
TestResult run_linear_ineq_test1() {
  ...
  try {
    NLF1 nlp(
        2, linear_ineq_test1_obj, init_linear_ineq_test1, create_linear_ineq_test1_constraints
    );
    ...
    result.final_objective = nlp.getF();
    result.constraint_violation =
        check_constraint_violation(result.final_point, create_linear_ineq_test1_constraints(2));
    result.iterations = optimizer.getIter();
...
TestResult run_linear_ineq_test2() {
  ...
  try {
    NLF1 nlp(
        3, linear_ineq_test2_obj, init_linear_ineq_test2, create_linear_ineq_test2_constraints
    );
    ...
    result.final_objective = nlp.getF();
    result.constraint_violation =
        check_constraint_violation(result.final_point, create_linear_ineq_test2_constraints(3));
    result.iterations = optimizer.getIter();
...
TestResult run_bounds_test2() {
  ...
  try {
    ...
    // Check bounds satisfaction
    bool bounds_ok =
        (result.final_point(0) >= -3.0 - 1e-6 && result.final_point(0) <= 1.0 + 1e-6) &&
...
TestResult run_mixed_linear_test1() {
  ...
  try {
    NLF1 nlp(
        3, mixed_linear_test1_obj, init_mixed_linear_test1, create_mixed_linear_test1_constraints
    );
    ...
    result.final_objective = nlp.getF();
    result.constraint_violation =
        check_constraint_violation(result.final_point, create_mixed_linear_test1_constraints(3));
    result.iterations = optimizer.getIter();
...
void nonlinear_eq_test1_con(
    int mode, int n, const SerialDenseVector<int, double>& x, SerialDenseVector<int, double>& fx,
    SerialDenseMatrix<int, double>& g, int& result
) {
...
CompoundConstraint* create_nonlinear_ineq_test1_constraints(int n) {
  ...
  NLP* constraint_nlp =
      new NLP(new NLF1(n, 1, nonlinear_ineq_test1_con, init_nonlinear_ineq_test1));
...
void mixed_nonlinear_test1_obj(
    int mode, int n, const SerialDenseVector<int, double>& x, double& fx,
    SerialDenseVector<int, double>& g, int& result
) {
  if (n != 3)
...
void mixed_nonlinear_test1_con(
    int mode, int n, const SerialDenseVector<int, double>& x, SerialDenseVector<int, double>& fx,
    SerialDenseMatrix<int, double>& g, int& result
) {
...
CompoundConstraint* create_mixed_nonlinear_test1_constraints(int n) {
  ...
  NLP* constraint_nlp =
      new NLP(new NLF1(n, 2, mixed_nonlinear_test1_con, init_mixed_nonlinear_test1));
  ...
  Constraint nleqn =
      new NonLinearEquation(constraint_nlp, rhs, 1); // Only first constraint is equality
...
void diagnostic_single_matrix_obj(
    int mode, int n, const SerialDenseVector<int, double>& x, double& fx,
    SerialDenseVector<int, double>& g, int& result
) {
  if (n != 3)
...
TestResult run_diagnostic_single_matrix() {
  ...
  try {
    NLF1 nlp(
        3, diagnostic_single_matrix_obj, init_diagnostic_single_matrix,
        create_diagnostic_single_matrix_constraints
    );
    ...
    result.final_point = nlp.getXc();
    result.final_objective = nlp.getF();
    result.constraint_violation = check_constraint_violation(
        result.final_point, create_diagnostic_single_matrix_constraints(3)
    );
    result.iterations = optimizer.getIter();
...
void diagnostic_separate_compounds_obj(
    int mode, int n, const SerialDenseVector<int, double>& x, double& fx,
    SerialDenseVector<int, double>& g, int& result
) {
  if (n != 3)
...
void diagnostic_two_constraints_obj(
    int mode, int n, const SerialDenseVector<int, double>& x, double& fx,
    SerialDenseVector<int, double>& g, int& result
) {
  if (n != 3)
...
TestResult run_diagnostic_two_constraints() {
  ...
  try {
    NLF1 nlp(
        3, diagnostic_two_constraints_obj, init_diagnostic_two_constraints,
        create_diagnostic_two_constraints_constraints
    );
    ...
    result.final_objective = nlp.getF();
    result.constraint_violation = check_constraint_violation(
        result.final_point, create_diagnostic_two_constraints_constraints(3)
    );
    result.iterations = optimizer.getIter();
...
void diagnostic_one_constraint_obj(
    int mode, int n, const SerialDenseVector<int, double>& x, double& fx,
    SerialDenseVector<int, double>& g, int& result
) {
  if (n != 3)
...
TestResult run_diagnostic_one_constraint() {
  ...
  try {
    NLF1 nlp(
        3, diagnostic_one_constraint_obj, init_diagnostic_one_constraint,
        create_diagnostic_one_constraint_constraints
    );
    ...
    result.final_objective = nlp.getF();
    result.constraint_violation = check_constraint_violation(
        result.final_point, create_diagnostic_one_constraint_constraints(3)
    );
    result.iterations = optimizer.getIter();
...
void diagnostic_different_start_obj(
    int mode, int n, const SerialDenseVector<int, double>& x, double& fx,
    SerialDenseVector<int, double>& g, int& result
) {
  if (n != 3)
...
TestResult run_diagnostic_different_start() {
  ...
  try {
    NLF1 nlp(
        3, diagnostic_different_start_obj, init_diagnostic_different_start,
        create_diagnostic_different_start_constraints
    );
    ...
    result.final_objective = nlp.getF();
    result.constraint_violation = check_constraint_violation(
        result.final_point, create_diagnostic_different_start_constraints(3)
    );
    result.iterations = optimizer.getIter();
...
void diagnostic_skip_first_obj(
    int mode, int n, const SerialDenseVector<int, double>& x, double& fx,
    SerialDenseVector<int, double>& g, int& result
) {
  if (n != 3)
...
TestResult run_diagnostic_skip_first() {
  ...
  try {
    NLF1 nlp(
        3, diagnostic_skip_first_obj, init_diagnostic_skip_first,
        create_diagnostic_skip_first_constraints
    );
    ...
    result.final_objective = nlp.getF();
    result.constraint_violation =
        check_constraint_violation(result.final_point, create_diagnostic_skip_first_constraints(3));
    result.iterations = optimizer.getIter();
...
void diagnostic_simple_bounds_obj(
    int mode, int n, const SerialDenseVector<int, double>& x, double& fx,
    SerialDenseVector<int, double>& g, int& result
) {
  if (n != 3)
...
TestResult run_diagnostic_simple_bounds() {
  ...
  try {
    NLF1 nlp(
        3, diagnostic_simple_bounds_obj, init_diagnostic_simple_bounds,
        create_diagnostic_simple_bounds_constraints
    );
    ...
    result.final_objective = nlp.getF();
    result.constraint_violation = check_constraint_violation(
        result.final_point, create_diagnostic_simple_bounds_constraints(3)
    );
    result.iterations = optimizer.getIter();
...
void diagnostic_detailed_eval_obj(
    int mode, int n, const SerialDenseVector<int, double>& x, double& fx,
    SerialDenseVector<int, double>& g, int& result
) {
  if (n != 3)
...
TestResult run_diagnostic_detailed_eval() {
  ...
  try {
    NLF1 nlp(
        3, diagnostic_detailed_eval_obj, init_diagnostic_detailed_eval,
        create_diagnostic_detailed_eval_constraints
    );
    ...
    if (constraints) {
      try {
        ...
        result.message =
            result.constraint_violation < 1e-5 ? "PASSED - " + msg.str() : "FAILED - " + msg.str();
      } catch (...) {
        result.message =
            result.constraint_violation < 1e-5 ? "PASSED" : "FAILED - constraint violation";
      }
    } else {
...
TestResult run_diagnostic_manual_evaluation() {
  ...
  try {
    ...
    std::ostringstream msg;
    msg << std::scientific << std::setprecision(6);
    msg << "Origin[0,0,0]: " << origin_violation << ", Feasible[0.5,0.5,0]: " << feasible_violation
...
double check_constraint_violation(
    const SerialDenseVector<int, double>& x, CompoundConstraint* constraints
) {
  if (!constraints)
    return 0.0;
...
void print_test_result(const std::string& test_name, const TestResult& result) {
  ...
  std::cout << " | f = " << std::setw(8) << std::fixed << std::setprecision(4)
            << result.final_objective;
  std::cout << " | violation = " << std::setw(8) << std::scientific << std::setprecision(2)
            << result.constraint_violation;
