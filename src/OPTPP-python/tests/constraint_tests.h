...
struct AllTestResults {
  ...
  int passed_tests() const {
    int count = 0;
    if (linear_eq_test1.success)
      count++;
    if (linear_eq_test2.success)
      count++;
    if (linear_ineq_test1.success)
      count++;
    if (linear_ineq_test2.success)
      count++;
    if (bounds_test1.success)
      count++;
    if (bounds_test2.success)
      count++;
    if (mixed_linear_test1.success)
      count++;
    if (nonlinear_eq_test1.success)
      count++;
    if (nonlinear_ineq_test1.success)
      count++;
    if (mixed_nonlinear_test1.success)
      count++;
    if (diagnostic_single_matrix.success)
      count++;
    if (diagnostic_separate_compounds.success)
      count++;
    if (diagnostic_two_constraints.success)
      count++;
    if (diagnostic_one_constraint.success)
      count++;
    if (diagnostic_different_start.success)
      count++;
    if (diagnostic_skip_first.success)
      count++;
    if (diagnostic_simple_bounds.success)
      count++;
    if (diagnostic_detailed_eval.success)
      count++;
    if (diagnostic_manual_evaluation.success)
      count++;
    return count;
  }
};
