...
public:
  ...
  static double norm(const Vector& v) { return std::sqrt(dot(v, v)); }
...
public:
  OptQNewton(py::function func, int dimension)
      : objective_func(func), n(dimension), x_current(dimension), x_previous(dimension),
        ...
        fcn_tol(1e-6), grad_tol(1e-6), max_iter(100), max_feval(1000), iteration(0), func_evals(0),
        grad_evals(0), debug_output(false) {}
  ...
  Matrix updateHessian(const Matrix& H, int k) {
    ...
    if (k == 0) {
      ...
      if (debug_output) {
        std::cout << "Initial Hessian scaling: " << scaling << ", gnorm: " << gnorm
                  << ", typx: " << typx << std::endl;
      }
    ...
    if (debug_output) {
      std::cout << "BFGS update: y^T*s = " << yts << ", ||s|| = " << snorm << ", ||y|| = " << ynorm
                << std::endl;
    }
  ...
  py::dict optimize(const Vector& x0) {
    ...
    if (debug_output) {
      ...
      std::cout << "Initial f = " << f_current << ", ||g|| = " << MatrixOps::norm(grad_current)
                << std::endl;
    }
    ...
    for (iteration = 1; iteration <= max_iter; iteration++) {
      ...
      if (debug_output) {
        std::cout << "Iter " << iteration << ": f = " << f_current
                  << ", ||g|| = " << MatrixOps::norm(grad_current) << ", alpha = " << alpha
                  << std::endl;
      }
    }
...
PYBIND11_MODULE(everest_optimizers, m) {
  ...
  m.def(
      "optimize_python_func",
      [](py::function func, std::vector<double> x0) { return optimize_with_optqnewton(func, x0); },
      "Optimize a Python function using OPTPP's OptQNewton algorithm", py::arg("func"),
      py::arg("x0")
  );
  ...
  m.def(
      ...
      []() {
        ...
        py::function py_func =
            py::cpp_function([func](const std::vector<double>& x) { return func(x); });
